...
비슷한 적용 사례는 VNC 적용으로 사용 불편 및 가상 머신으로 이용자 불편이 매우 공존하고 있습니다.

파일럿으로 오픈소스의 웹 기반을 적용하였으며, 내부 테스트 검증용 진행하고 있습니다.

[참조]
오렌지 3 : https://orangedatamining.com/
깃허브 : https://github.com/biolab/orange3

-사용자 편의성과 웹 전환의 목표로 진행
-적용시 많은 오류에 직면하여, 수많은 수정
-개선 버전 아래 URL을 통해, PC 버전과 비교 사용 가능
...
피시 설치형 데이터 분석 프로그램 Orange3를 웹 프로그램으로 파일럿 제작하였습니다.

기본 구조만 작업 하였으며, 위젯과 데이터 분석 로직 부분은 추가 확장은 연속으로 진행이 필요합니다.

파일썬으로 하던 GUI 프로그램에서 AI 분석 프로그램 고도화를 위한 오픈소스 프로젝트 입니다.

- Orange3 웹 배포
— Docker 기반 한국어 Orange3 GUI를 브라우저에서 다중 세션 제공
- 세센 분리가 안되는 부분
- 화면 리플래쉬가 동작 오류를 개선해 나가고 있습니다.
- 각 사용자에게 격리된 Orange3 컨테이너를 동적으로 할당
- 초기 세팅 과정 나오지 않는 부분 개발 과정 다른 이슈로, 계속 보완 중임
- 화면을 noVNC(WebSocket RFB)로 브라우저에 전송한다. 
...
4. VS CODE 사용(깃허브는 배포용으로 마지막에 사용하였습니다.)
...
- 사용자 증가시 메모리 증가 : 서버 최적화 진행하였으나, 오렌지 3의 캔버스 사용시 메모리 증가
- 위와 동일한 원인으로 서버 콘테이너가 사용자,풀 필요로, 이용자가 작은 시스템에서 제공가능
- 언어 변경시 시간 지연 문제 발생
