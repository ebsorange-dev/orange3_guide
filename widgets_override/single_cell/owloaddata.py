...
class OWLoadData(widget.OWWidget):
    ...
    def __init__(self):
        ...
        # ── 브라우저 PC 파일 선택기 폴링 (2026-05-28, owfile.py 패턴) ─────────
        # browse_row_annotations / browse_col_annotations 가 QFileDialog 대신
        # 신호 파일 작성 → 세션매니저 SSE → 브라우저 파일 선택기 호출.
        # 업로드 완료 후 컨테이너의 .upload_path_sc_cell_anno / _sc_gene_anno 신호
        # 파일을 1.5s 주기로 폴링하여 자동 적용.
        self._sc_anno_poll_timer = QTimer(self)
        self._sc_anno_poll_timer.setInterval(1500)
        self._sc_anno_poll_timer.timeout.connect(self._poll_sc_anno_uploads)
        self._sc_anno_poll_timer.start()
    ...
    def browse_row_annotations(self):
        """Cell annotations 선택 — Qt 다이얼로그 대신 브라우저 PC 파일 선택기 호출.
        세션매니저가 SSE 로 감지 → 브라우저 file input 클릭 → 업로드 완료 후
        /config/.upload_path_sc_cell_anno 신호 파일 → _poll_sc_anno_uploads() 적용."""
        import time as _time
        try:
            with open("/config/.upload_request_sc_cell_anno", "w") as _f:
    ...
    def browse_col_annotations(self):
        """Gene annotations 선택 — Cell annotations 와 동일 패턴."""
        import time as _time
        try:
            with open("/config/.upload_request_sc_gene_anno", "w") as _f:
                _f.write(str(_time.time()))
        except OSError:
            pass

    def _poll_sc_anno_uploads(self):
        """Cell / Gene annotation 업로드 완료 신호 폴링 (1.5s 주기).
        owfile.py 의 _poll_uploads() 와 동일 패턴 — 신호 파일 발견 시 적용 후 제거."""
        # Cell annotations (row)
        sig_row = "/config/.upload_path_sc_cell_anno"
        if os.path.isfile(sig_row):
            try:
                with open(sig_row) as _f:
                    path = _f.read().strip()
                os.remove(sig_row)
            except OSError:
                path = None
            if path and os.path.isfile(path):
                ...
                pathitem = RecentPath.create(path, [])
                index = insert_recent_path(m, pathitem)
                self.row_annotations_combo.setCurrentIndex(index)
                self._row_annotations_combo_changed()
                self._invalidate()
        # Gene annotations (col)
        sig_col = "/config/.upload_path_sc_gene_anno"
        if os.path.isfile(sig_col):
            try:
                with open(sig_col) as _f:
                    path = _f.read().strip()
                os.remove(sig_col)
            except OSError:
                path = None
