...
class ParameterSetter(CommonParameterSetter):
    BOTTOM_AXIS_LABEL, IS_VERTICAL_LABEL = _tr.m[3953, "Bottom axis"], _tr.m[3954, "Vertical tick text"]
...
class OWViolinPlot(OWWidget):
    name = _tr.m[3955, "Violin Plot"]
    description = (_tr.m[3956, "Visualize the distribution of feature"] + _tr.m[3957, " values in a violin plot."])
    icon = "icons/ViolinPlot.svg"
    priority = 110
    keywords = _tr.m[3958, "violin plot, kernel, density"]

    class Inputs:
        data = Input(_tr.m[3959, "Data"], Table, id="Data")

    class Outputs:
        selected_data = Output(_tr.m[3960, "Selected Data"], Table, id="Selected Data", default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, id="Data")

    class Error(OWWidget.Error):
        no_cont_features = Msg(_tr.m[3961, "Plotting requires a numeric feature."])
        not_enough_instances = Msg(_tr.m[3962, "Plotting requires at least two instances."])

    KERNELS = ["gaussian", "epanechnikov", "linear"]
    KERNEL_LABELS = [_tr.m[3963, "Normal"], _tr.m[3964, "Epanechnikov"], _tr.m[3965, "Linear"]]
    SCALE_LABELS = [_tr.m[3966, "Area"], _tr.m[3967, "Count"], _tr.m[3968, "Width"]]
    ...
    def _add_controls(self):
        ...
        self._group_var_model = VariableListModel(placeholder=_tr.m[3969, "None"])
        sorted_model = SortProxyModel(sortRole=Qt.UserRole)
        ...
        box = gui.vBox(self.controlArea, _tr.m[3970, "Variable"])
        box.layout().addWidget(self._value_var_view)
        gui.checkBox(box, self, "order_by_importance",
                     _tr.m[3971, "Order by relevance to subgroups"],
                     tooltip=_tr.m[3972, "Order by 𝜒² or ANOVA over the subgroups"],
                     callback=self.apply_value_var_sorting)

        box = gui.vBox(self.controlArea, _tr.m[3973, "Subgroups"])
        box.layout().addWidget(self._group_var_view)
        gui.checkBox(box, self, "order_grouping_by_importance",
                     _tr.m[3974, "Order by relevance to variable"],
                     tooltip=_tr.m[3975, "Order by 𝜒² or ANOVA over the variable values"],
                     callback=self.apply_group_var_sorting)

        box = gui.vBox(self.controlArea, _tr.m[3976, "Display"],
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.checkBox(box, self, "show_box_plot", _tr.m[3977, "Box plot"],
                     callback=self.__show_box_plot_changed)
        gui.checkBox(box, self, "show_strip_plot", _tr.m[3978, "Density dots"],
                     callback=self.__show_strip_plot_changed)
        gui.checkBox(box, self, "show_rug_plot", _tr.m[3979, "Density lines"],
                     callback=self.__show_rug_plot_changed)
        self._order_violins_cb = gui.checkBox(
            box, self, "order_violins", _tr.m[3980, "Order subgroups"],
            callback=self.__order_violins_changed,
        )
        gui.checkBox(
            box, self, "show_grid", _tr.m[3981, "Show grid"],
            callback=self.__show_grid_changed,
        )
        gui.radioButtons(box, self, "orientation_index",
                         [_tr.m[3982, "Horizontal"], _tr.m[3983, "Vertical"]], label=_tr.m[3984, "Orientation: "],
                         orientation=Qt.Horizontal,
                         callback=self.__orientation_changed)

        box = gui.vBox(self.controlArea, _tr.m[3985, "Density Estimation"],
                       sizePolicy=(QSizePolicy.Minimum, QSizePolicy.Maximum))
        gui.comboBox(box, self, "kernel_index", items=self.KERNEL_LABELS,
                     label=_tr.m[3986, "Kernel:"], labelWidth=60, orientation=Qt.Horizontal,
                     callback=self.__kernel_changed)
        self._scale_combo = gui.comboBox(
            box, self, "scale_index", items=self.SCALE_LABELS,
            label=_tr.m[3987, "Scale:"], labelWidth=60, orientation=Qt.Horizontal,
