...
class ParameterSetter(CommonParameterSetter):
    MEAN_LABEL = _tr.m[3607, "Mean"]
    LINE_LABEL = _tr.m[3608, "Lines"]
    MISSING_LINE_LABEL = _tr.m[3609, "Lines (missing value)"]
    SEL_LINE_LABEL = _tr.m[3610, "Selected lines"]
    SEL_MISSING_LINE_LABEL = _tr.m[3611, "Selected lines (missing value)"]
    RANGE_LABEL = _tr.m[3612, "Range"]
    SEL_RANGE_LABEL = _tr.m[3613, "Selected range"]
    ...
    def update_setters(self):
        ...
        self.missing_line_settings = {
            Updater.WIDTH_LABEL: LinePlotStyle.UNSELECTED_LINE_WIDTH,
            Updater.ALPHA_LABEL: LinePlotStyle.UNSELECTED_LINE_ALPHA,
            Updater.STYLE_LABEL: _tr.m[3614, "Dash line"],
            Updater.ANTIALIAS_LABEL: True,
        }
        ...
        self.sel_missing_line_settings = {
            Updater.WIDTH_LABEL: LinePlotStyle.SELECTED_LINE_WIDTH,
            Updater.ALPHA_LABEL: LinePlotStyle.SELECTED_LINE_ALPHA,
            Updater.STYLE_LABEL: _tr.m[3615, "Dash line"],
            Updater.ANTIALIAS_LABEL: False,
        }
        self.range_settings = {
...
class OWLinePlot(OWWidget):
    name = _tr.m[3618, "Line Plot"]
    description = _tr.m[3619, "Visualization of data profiles (e.g., time series)."]
    icon = "icons/LinePlot.svg"
    priority = 180
    keywords = _tr.m[3620, "line plot"]

    buttons_area_orientation = Qt.Vertical
    enable_selection = Signal(bool)

    class Inputs:
        data = Input(_tr.m[3621, "Data"], Table, id="Data", default=True)
        data_subset = Input(_tr.m[3622, "Data Subset"], Table, id="Data Subset")

    class Outputs:
        selected_data = Output(_tr.m[3623, "Selected Data"], Table, id="Selected Data", default=True)
    ...
    class Error(OWWidget.Error):
        not_enough_attrs = Msg(_tr.m[3624, "Need at least one numeric feature."])

    class Warning(OWWidget.Warning):
        no_display_option = Msg(_tr.m[3625, "No display option is selected."])

    class Information(OWWidget.Information):
        too_many_features = Msg((_tr.m[3626, "Data has too many features. Only first {}"] + _tr.m[3627, " are shown."]).format(MAX_FEATURES))
    ...
    def _add_controls(self):
        displaybox = gui.widgetBox(self.controlArea, _tr.m[3628, "Display"])
        gui.checkBox(displaybox, self, "show_profiles", _tr.m[3629, "Lines"],
                     callback=self.__show_profiles_changed,
                     tooltip=_tr.m[3630, "Plot lines"])
        gui.checkBox(displaybox, self, "show_range", _tr.m[3631, "Range"],
                     callback=self.__show_range_changed,
                     tooltip=_tr.m[3632, "Plot range between 10th and 90th percentile"])
        gui.checkBox(displaybox, self, "show_mean", _tr.m[3633, "Mean"],
                     callback=self.__show_mean_changed,
                     tooltip=_tr.m[3634, "Plot mean curve"])
        gui.checkBox(displaybox, self, "show_error", _tr.m[3635, "Error bars"],
                     callback=self.__show_error_changed,
                     tooltip=_tr.m[3636, "Show standard deviation"])

        self.group_vars = DomainModel(
            placeholder=_tr.m[3637, "None"], separators=False, valid_types=DiscreteVariable)
        self.group_view = gui.listView(
            self.controlArea, self, "group_var", box=_tr.m[3638, "Group by"],
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert(((_tr.m[3639, "Group by"], self.group_var),))
        self.report_plot()
        if caption:
