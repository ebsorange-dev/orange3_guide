...
class OWRuleViewer(widget.OWWidget):
    name = _tr.m[3764, "CN2 Rule Viewer"]
    description = _tr.m[3765, "Review rules induced from data."]
    icon = "icons/CN2RuleViewer.svg"
    priority = 1140
    keywords = _tr.m[3766, "cn2 rule viewer"]

    class Inputs:
        data = Input(_tr.m[3767, "Data"], Table, id="Data")
        classifier = Input(_tr.m[3768, "Classifier"], _RuleClassifier, id="Classifier")

    class Outputs:
        selected_data = Output(_tr.m[3769, "Selected Data"], Table, id="Selected Data", default=True)
    ...
    def __init__(self):
        ...
        self.model.set_horizontal_header_labels(
            [_tr.m[3770, "IF conditions"], "", _tr.m[3771, "THEN class"], _tr.m[3772, "Distribution"],
             _tr.m[3773, "Probabilities [%]"], _tr.m[3774, "Quality"], _tr.m[3775, "Length"]])
        ...
        gui.checkBox(widget=self.buttonsArea, master=self, value="compact_view",
                     label=_tr.m[3776, "Compact view"], callback=self.on_update)
        gui.rubber(self.buttonsArea)

        original_order_button = gui.button(
            self.buttonsArea, self,
            _tr.m[3777, "Restore original order"],
            autoDefault=False,
    ...
    def send_report(self):
        if self.classifier is not None:
            self.report_table(_tr.m[3778, "Induced rules"], self.view)
...
class CustomRuleViewerTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        def _display_role():
            if column == 0:
                delim = _tr.m[3779, " AND "] if self._compact_view else _tr.m[3780, " AND\n"]
                return _tr.m[3781, "TRUE"] if not rule.selectors else delim.join(
        ...
        def _tooltip_role():
            if column == 0:
                return _display_role().replace(_tr.m[3782, " AND "], _tr.m[3783, " AND\n"])
            if column == 1:
                return None
            if column == 3:
                # list of int, float
