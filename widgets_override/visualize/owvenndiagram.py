...
from AnyQt.QtCore import Qt, QSize, QPointF, QRectF, QLineF
...
IDENTITY_STR = _tr.m[3932, "Instance identity"]
EQUALITY_STR = _tr.m[3933, "Instance equality"]
...
class OWVennDiagram(widget.OWWidget):
    name = _tr.m[3934, "Venn Diagram"]
    description = (_tr.m[3935, "A graphical visualization of the overlap of data instances "] + _tr.m[3936, "from a collection of input datasets."])
    icon = "icons/VennDiagram.svg"
    priority = 280
    keywords = _tr.m[3937, "venn diagram"]
    settings_version = 2

    class Inputs:
        data = MultiInput(_tr.m[3938, "Data"], Table, id="Data")

    class Outputs:
        selected_data = Output(_tr.m[3939, "Selected Data"], Table, id="Selected Data", default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, id="Data")

    class Error(widget.OWWidget.Error):
        instances_mismatch = Msg(_tr.m[3940, "Data sets do not contain the same instances."])
        too_many_inputs = Msg(_tr.m[3941, "Venn diagram accepts at most five datasets."])

    class Warning(widget.OWWidget.Warning):
        renamed_vars = Msg((_tr.m[3942, "Some variables have been renamed "] + _tr.m[3943, "to avoid duplicates.\n{}"]))
    ...
    def __init__(self):
        ...
        box = gui.radioButtonsInBox(
            ...
            [_tr.m[3944, "Columns (features)"], _tr.m[3945, "Rows (instances), matched by"], ],
        ...
        gui.comboBox(
            ...
            tooltip=(_tr.m[3946, "Instances are identical if originally coming from the "] + (_tr.m[3947, "same row of the same table.\n"] + (_tr.m[3948, "Instances can be check for equality only if described by "] + _tr.m[3949, "the same variables."]))))
        ...
        self.output_duplicates_cb = gui.checkBox(
            box, self, "output_duplicates", _tr.m[3950, "Output duplicates"],
        ...
        for i, area in enumerate(self.vennwidget.vennareas()):
            ...
            if self._uses_feature() or not self.rowwise:
                ...
                if len(area_items) > 32:
                    tooltip += _tr.e(_tr.c(3952, f"</br>({len(area_items) - 32} items not shown)"))
                tooltip += "</span>"
