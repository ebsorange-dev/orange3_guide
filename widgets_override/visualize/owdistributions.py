...
class OWDistributions(OWWidget):
    name = _tr.m[3432, "Distributions"]
    description = _tr.m[3433, "Display value distributions of a data feature in a graph."]
    icon = "icons/Distribution.svg"
    priority = 120
    keywords = _tr.m[3434, "distributions, histogram"]

    class Inputs:
        data = Input(_tr.m[3435, "Data"], Table, id="Data", doc=_tr.m[3436, "Set the input dataset"])

    class Outputs:
        selected_data = Output(_tr.m[3437, "Selected Data"], Table, id="Selected Data", default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, id="Data")
        histogram_data = Output(_tr.m[3438, "Histogram Data"], Table, id="Histogram Data", dynamic=False)

    class Error(OWWidget.Error):
        no_defined_values_var = \
            Msg(_tr.m[3439, "Variable '{}' does not have any defined values"])
        no_defined_values_pair = \
            Msg(_tr.m[3440, "No data instances with '{}' and '{}' defined"])

    class Warning(OWWidget.Warning):
        ignored_nans = Msg(_tr.m[3441, "Data instances with missing values are ignored"])
    ...
    Fitters = (
        (_tr.m[3442, "None"], None, (), ()),
        (_tr.m[3443, "Normal"], norm, ("loc", "scale"), ("μ", "σ")),
        ("Beta", beta, ("a", "b", "loc", "scale"),
         ("α", "β", "-loc", "-scale")),
        (_tr.m[3444, "Gamma"], gamma, ("a", "loc", "scale"), ("α", "β", "-loc", "-scale")),
        ("Rayleigh", rayleigh, ("loc", "scale"), ("-loc", "σ")),
        ("Pareto", pareto, ("b", "loc", "scale"), ("α", "-loc", "-scale")),
        (_tr.m[3445, "Exponential"], expon, ("loc", "scale"), ("-loc", "λ")),
        (_tr.m[3446, "Kernel density"], AshCurve, ("a",), ("",))
    )
    ...
    def __init__(self):
        ...
        # 플로피 디스크(save image) 아이콘 숨김
        action = getattr(self, '_OWBaseWidget__save_image_action', None)
        ...
        varview = gui.listView(
            self.controlArea, self, "var", box=_tr.m[3447, "Variable"],
        ...
        gui.checkBox(
            varview.box, self, "sort_by_freq", _tr.m[3448, "Sort categories by frequency"],
            callback=self._on_sort_by_freq, stateWhenDisabled=False)

        box = self.continuous_box = gui.vBox(self.controlArea, _tr.m[3449, "Distribution"])
        gui.comboBox(
            box, self, "fitted_distribution", label=_tr.m[3450, "Fitted distribution"],
        ...
        slider = gui.hSlider(
            ...
            label=_tr.m[3451, "Bin width"], orientation=Qt.Horizontal,
        ...
        self.smoothing_box = gui.hSlider(
            ...
            label=_tr.m[3452, "Smoothing"], orientation=Qt.Horizontal,
            minValue=2, maxValue=20, callback=self.replot, disabled=True)
        gui.checkBox(
            box, self, "hide_bars", _tr.m[3453, "Hide bars"], stateWhenDisabled=False,
            callback=self._on_hide_bars_changed,
            disabled=not self.fitted_distribution)

        box = gui.vBox(self.controlArea, _tr.m[3454, "Columns"])
        gui.comboBox(
            box, self, "cvar", label=_tr.m[3455, "Split by"], orientation=Qt.Horizontal,
            searchable=True,
            model=DomainModel(placeholder=_tr.m[3456, "(None)"],
                              valid_types=(DiscreteVariable), ),
            callback=self._on_cvar_changed, contentsLength=18)
        gui.checkBox(
            box, self, "stacked_columns", _tr.m[3457, "Stack columns"],
            callback=self.replot)
        gui.checkBox(
            box, self, "show_probs", _tr.m[3458, "Show probabilities"],
            callback=self._on_show_probabilities_changed)
        gui.checkBox(
            box, self, "cumulative_distr", _tr.m[3459, "Show cumulative distribution"],
    ...
    def _on_show_probabilities_changed(self):
        ...
        if self.show_probs:
            label.setText(_tr.m[3460, "Fitted probability"])
            label.setToolTip(
                _tr.m[3461, "Chosen distribution is used to compute Bayesian probabilities"])
        else:
            label.setText(_tr.m[3462, "Fitted distribution"])
            label.setToolTip("")
        self.replot()
    ...
    def _set_axis_names(self):
        ...
        if self.show_probs and self.cvar:
            leftaxis.setLabel(
                _tr.e(_tr.c(3463, f"Probability of '{self.cvar.name}' at given '{self.var.name}'")))
        else:
            leftaxis.setLabel(_tr.m[3464, "Frequency"])
        ...
        if self.is_valid:
            ...
            if self.var.is_continuous:  # annotate with bins
                ...
                annotated_data = create_groups_table(
                    annotated_data, hist_indices, var_name=_tr.m[3465, "Bin"], values=hist_values)
    ...
    def _get_histogram_table(self):
        ...
        var_bin = DiscreteVariable(_tr.m[3466, "Bin"], [bar.desc for bar in self.bar_items])
        var_freq = ContinuousVariable(_tr.m[3467, "Count"])
        X = []
        if self.cvar:
    ...
    def send_report(self):
        ...
        self.report_plot()
        if self.cumulative_distr:
            text = _tr.e(_tr.c(3468, f"Cummulative distribution of '{self.var.name}'"))
        else:
            text = _tr.e(_tr.c(3469, f"Distribution of '{self.var.name}'"))
        if self.cvar:
            text += _tr.e(_tr.c(3470, f" with columns split by '{self.cvar.name}'"))
