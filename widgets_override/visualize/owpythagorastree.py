...
class OWPythagorasTree(OWWidget):
    name = _tr.m[3706, "Pythagorean Tree"]
    description = _tr.m[3707, 'Pythagorean Tree visualization for tree like-structures.']
    icon = 'icons/PythagoreanTree.svg'
    keywords = _tr.m[3708, "pythagorean tree, fractal"]

    priority = 1000

    class Inputs:
        tree = Input(_tr.m[3709, "Tree"], TreeModel, id="Tree")

    class Outputs:
        selected_data = Output(_tr.m[3710, "Selected Data"], Table, id="Selected Data", default=True)
    ...
    def __init__(self):
        ...
        self.SIZE_CALCULATION = [
            (_tr.m[3711, 'Normal'], lambda x: x),
            (_tr.m[3712, 'Square root'], lambda x: sqrt(x)),
            (_tr.m[3713, 'Logarithmic'], lambda x: log(x * self.size_log_scale + 1)),
        ]

        # CONTROL AREA
        # Tree info area
        box_info = gui.widgetBox(self.controlArea, _tr.m[3714, 'Tree Info'])
        self.infolabel = gui.widgetLabel(box_info)

        # Display settings area
        box_display = gui.widgetBox(self.controlArea, _tr.m[3715, 'Display Settings'])
        ...
        self.log_scale_box = gui.hSlider(
            ...
            label=_tr.m[3719, 'Log scale factor'], minValue=1, maxValue=100, ticks=False,
            callback=self.invalidate_tree)

        # Plot properties area
        box_plot = gui.widgetBox(self.controlArea, _tr.m[3720, 'Plot Properties'])
        self.cb_show_tooltips = gui.checkBox(
            box_plot, self, 'tooltips_enabled', label=_tr.m[3721, 'Enable tooltips'],
            callback=self.update_tooltip_enabled)
        self.cb_show_legend = gui.checkBox(
            box_plot, self, 'show_legend', label=_tr.m[3722, 'Show legend'],
            callback=self.update_show_legend)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, label=_tr.m[3723, "Redraw"], callback=self.redraw)
    ...
    def _update_info_box(self):
        self.infolabel.setText(_tr.m[3724, 'Nodes: {}\nDepth: {}'].format(
    ...
    def _update_log_scale_slider(self):
        ...
        self.log_scale_box.parent().setEnabled(
            self.SIZE_CALCULATION[self.size_calc_idx][0] == _tr.m[3725, 'Logarithmic'])

    def _clear_info_box(self):
        self.infolabel.setText(_tr.m[3726, 'No tree on input'])
    ...
    def _update_target_class_combo(self):
        ...
        if self.data.domain.has_discrete_class:
            label_text = _tr.m[3727, 'Target class']
            values = [c.title() for c in self.data.domain.class_vars[0].values]
            values.insert(0, _tr.m[3728, 'None'])
        else:
            label_text = _tr.m[3729, 'Node color']
