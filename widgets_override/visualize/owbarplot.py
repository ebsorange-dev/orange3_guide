...
class ParameterSetter(CommonParameterSetter):
    GRID_LABEL, SHOW_GRID_LABEL = _tr.m[3381, "Gridlines"], _tr.m[3382, "Show"]
    DEFAULT_ALPHA_GRID, DEFAULT_SHOW_GRID = 80, True
    BOTTOM_AXIS_LABEL, GROUP_AXIS_LABEL = _tr.m[3383, "Bottom axis"], _tr.m[3384, "Group axis"]
    IS_VERTICAL_LABEL = _tr.m[3385, "Vertical ticks"]
...
class OWBarPlot(OWWidget):
    name = _tr.m[3386, "Bar Plot"]
    description = _tr.m[3387, "Visualizes comparisons among categorical variables."]
    icon = "icons/BarPlot.svg"
    priority = 190
    keywords = _tr.m[3388, "bar plot, chart"]

    class Inputs:
        data = Input(_tr.m[3389, "Data"], Table, id="Data", default=True)
        data_subset = Input(_tr.m[3390, "Data Subset"], Table, id="Data Subset")

    class Outputs:
        selected_data = Output(_tr.m[3391, "Selected Data"], Table, id="Selected Data", default=True)
    ...
    class Error(OWWidget.Error):
        no_cont_features = Msg(_tr.m[3392, "Plotting requires a numeric feature."])

    class Information(OWWidget.Information):
        too_many_instances = Msg((_tr.m[3393, "Data has too many instances. Only first {}"] + _tr.m[3394, " are shown."]).format(MAX_INSTANCES))

    enumeration = _tr.m[3395, "Enumeration"]
    ...
    def _add_controls(self):
        ...
        gui.comboBox(
            box, self, "selected_var", label=_tr.m[3396, "Values:"],
        ...
        self._group_var_model = DomainModel(
            placeholder=_tr.m[3397, "None"], valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "group_var", label=_tr.m[3398, "Group by:"],
        ...
        gui.comboBox(
            box, self, "annot_var", label=_tr.m[3400, "Annotations:"],
            model=self._annot_var_model, contentsLength=12, searchable=True,
            orientation=Qt.Horizontal, callback=self.__parameter_changed,
        )

        self._color_var_model = DomainModel(
            placeholder=_tr.m[3401, "(Same color)"], valid_types=DiscreteVariable
        )
        gui.comboBox(
            box, self, "color_var", label=_tr.m[3402, "Color:"],
