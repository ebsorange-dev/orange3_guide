...
class OWImageViewer(OWWidget):
    name = _tr.m[103, "Image Viewer"]
    description = _tr.m[104, "View images referred to in the data."]
    keywords = _tr.m[105, "image viewer, viewer, image"]
    icon = "icons/ImageViewer.svg"
    priority = 130
    replaces = ["Orange.widgets.data.owimageviewer.OWImageViewer", ]

    class Inputs:
        data = Input(_tr.m[106, "Data"], Orange.data.Table, id="Data")

    class Outputs:
        selected_data = Output(_tr.m[107, "Selected Images"], Orange.data.Table, id="Selected Images")
        data = Output(_tr.m[108, "Data"], Orange.data.Table, id="Data")

    class Warning(OWWidget.Warning):
        no_images_shown = Msg(
            ("Unable to display images. Check that the chosen " + "Image Filename Attribute stores correct paths to images.")
        )
    ...
    # Space 키 안내 메시지 노출 차단 — 빈 리스트로 비워둠
    UserAdviceMessages = []
    ...
    def __init__(self):
        ...
        gui.comboBox(
            ...
            self,
            "image_attr",
            box=_tr.m[111, "Image Filename Attribute"],
            tooltip=_tr.m[112, "Attribute with image filenames"],
        ...
        gui.comboBox(
            ...
            self,
            "title_attr",
            box=_tr.m[113, "Title Attribute"],
            tooltip=_tr.m[114, "Attribute with image title"],
        ...
        gui.hSlider(
            ...
            box=_tr.m[115, "Image Size"], minValue=32, maxValue=1024, step=16,
            callback=self.updateSize,
            createLabel=False
        )
        gui.rubber(self.controlArea)

        gui.auto_commit(self.buttonsArea, self, "autoCommit", _tr.m[116, "Send"], box=False)
...
def loader_local(
    ...
    def load_local_file() -> QImage:
        ...
        _log.debug(_tr.m[117, "Read local: %s"], reader.fileName())
        image = reader.read()
        if image.isNull():
...
def loader_qnam(
    ...
    def load_qnam() -> 'Future[QImage]':
        ...
        _log.debug(_tr.m[118, "Fetch: %s"], url.toString())
        ...
        def abort_on_cancel(f: Future) -> None:
            if f.cancelled():
                if not reply.isFinished():
                    _log.debug(_tr.m[119, "Abort: %s"], reply.url().toString())
                    reply.abort()
        ...
        def on_reply_finished():
            ...
            if _log.level <= logging.DEBUG:
                ...
                if reply.attribute(QNetworkRequest.SourceIsFromCacheAttribute):
                    print(_tr.m[120, "  (served from cache)"], file=s)
...
def image_loader(
    ...
    if url.isValid() and url.isLocalFile():
        ...
        future.set_exception(ValueError(_tr.e(_tr.c(121, "'{url.toString()}' is not a valid url"))))
    ...
    def get(self, url):
        ...
        def on_reply_ready(reply, future):
            ...
            if _log.level <= logging.DEBUG:
                ...
                print("\n", reply.url(), file=s)
                if reply.attribute(QNetworkRequest.SourceIsFromCacheAttribute):
                    print(_tr.m[122, "  (served from cache)"], file=s)
                for name, val in reply.rawHeaderPairs():
                    print(bytes(name).decode("latin-1"), ":",
                          bytes(val).decode("latin-1"), file=s)
                _log.debug(s.getvalue())
