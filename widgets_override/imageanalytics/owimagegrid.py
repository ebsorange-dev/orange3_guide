...
class OWImageGrid(widget.OWWidget):
    name = _tr.m[47, "Image Grid"]
    description = _tr.m[48, "Visualize images in a similarity grid"]
    icon = "icons/ImageGrid.svg"
    priority = 160
    keywords = _tr.m[49, "image grid, image, grid, similarity"]
    graph_name = "scene"  # QGraphicsScene

    class Inputs:
        data = Input(_tr.m[50, "Embeddings"], Orange.data.Table, id="Embeddings", default=True)
        data_subset = Input(_tr.m[51, "Data Subset"], Orange.data.Table, id="Data Subset")

    class Outputs:
        selected_data = Output(
            _tr.m[52, "Selected Images"], Orange.data.Table, id="Selected Images", default=True)
        data = Output(_tr.m[53, "Images"], Orange.data.Table, id="Images")

    settingsHandler = settings.DomainContextHandler()

    cell_fit = settings.Setting(_tr.m[54, "Resize"])
    ...
    class Warning(OWWidget.Warning):
        incompatible_subset = Msg(
            _tr.m[55, "No Data Subset elements appear in the Data."])
        extras_in_subset = Msg(
            _tr.m[56, "Some Data Subset elements do not appear in the Data."])
        no_valid_data = Msg(_tr.m[57, "No valid data"])
    ...
    def __init__(self):
        ...
        self.stringAttrs = []
        self.domainAttrs = []
        self.label_model = DomainModel(placeholder=_tr.m[58, "(No labels)"])
        ...
        self.imageAttrCB = gui.comboBox(
            ...
            box=_tr.m[59, "Image Filename Attribute"],
            tooltip=_tr.m[60, "Attribute with image filenames"],
        ...
        self.cellFitRB = gui.radioButtons(
            self.controlArea, self, "cell_fit", [_tr.m[61, "Resize"], _tr.m[62, "Crop"]],
            box=_tr.m[63, "Image cell fit"], callback=self.set_crop)

        self.gridSizeBox = gui.vBox(self.controlArea, _tr.m[64, "Grid size"])
        ...
        gui.button(
            self.gridSizeBox, self, _tr.m[67, "Set size automatically"],
            callback=self.auto_set_size)

        self.label_box = gui.vBox(self.controlArea, _tr.m[68, "Labels"])

        # labels control
        self.label_attr_cb = gui.comboBox(
            self.label_box, self, "label_attr",
            tooltip=_tr.m[69, "Show labels"],
            callback=self.update_size,
        ...
        # auto commit
        gui.auto_commit(self.buttonsArea, self, "auto_commit", _tr.m[70, "Apply"])
    ...
    def commit(self):
        ...
        if self.data and len(self.items) > 0:
            ...
            self.Outputs.selected_data.send(
                create_groups_table(
                    self.image_grid.image_list, self.selection, False, _tr.m[71, "Group"]
                )
    ...
    def send_report(self):
        if self.is_valid_data():
            items = [(_tr.m[72, "Number of images"], len(self.data))]
            self.report_items(items)
            self.report_plot(_tr.m[73, "Grid"], self.scene)
    ...
    def _note_completed(self, future):
        ...
        if future.exception():
            self._errcount += 1
            _log.debug(_tr.m[74, "Error: %r"], future.exception())
        else:
            self._successcount += 1
    ...
    def _update_status(self):
        ...
        if self._errcount + self._successcount == count:
            ...
            if self._errcount == count:
                self.error((_tr.e(_tr.c(75, "No images found for column '{attr.name}'! ")) + (_tr.e(_tr.c(76, "Make sure to usa a column with valid image path ")) + _tr.e(_tr.c(77, "name or urls.")))))
...
class GraphicsThumbnailGrid(QGraphicsWidget):
    ...
    def insertThumbnail(self, index, thumbnail):
        ...
        if isinstance(thumbnail, QPixmap):
            ...
            raise ValueError(_tr.m[78, "{!r} is already inserted"].format(thumbnail))
