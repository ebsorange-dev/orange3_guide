...
def permutation(
    ...
    scores_cv = [score_cv]
    correlations = [100.0]
    progress_callback(0, _tr.m[1912, "Calculating..."])
    np.random.seed(0)
...
class ParameterSetter(CommonParameterSetter):
    GRID_LABEL, SHOW_GRID_LABEL = _tr.m[1913, "Gridlines"], _tr.m[1914, "Show"]
...
class PermutationPlot(PlotWidget):
    def __init__(self):
        ...
        text = _tr.m[1915, "Correlation between original Y and permuted Y (%)"]
    ...
    def set_data(
        ...
        self.legend.clear()
        self.legend.addItem(pg.ScatterPlotItem(**kwargs_tr), _tr.m[1916, "Train"])
        self.legend.addItem(pg.ScatterPlotItem(**kwargs_cv), _tr.m[1917, "CV"])
        self.legend.show()


class OWPermutationPlot(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1918, "Permutation Plot"]
    description = _tr.m[1919, "Permutation analysis plotting"]
    ...
    class Inputs:
        data = Input(_tr.m[1920, "Data"], Table, id="Data")
        learner = Input(_tr.m[1921, "Learner"], Learner, id="Learner")

    class Error(OWWidget.Error):
        domain_transform_err = Msg("{}")
        unknown_err = Msg("{}")
        not_enough_data = Msg(_tr.e(_tr.c(1922, f"At least {N_FOLD} instances are needed.")))
    ...
    def _add_controls(self):
        box = gui.vBox(self.controlArea, _tr.m[1923, "Settings"])
        gui.spin(box, self, "n_permutations", label=_tr.m[1924, "Permutations:"],
                 minv=1, maxv=1000, callback=self._run)
        gui.rubber(self.controlArea)

        box = gui.vBox(self.controlArea, _tr.m[1925, "Info"])
        self._info = gui.label(box, self, "", textFormat=Qt.RichText,
                               minimumWidth=180)
        self.__set_info(None)

    def __set_info(self, result: PermutationResults):
        html = _tr.m[1926, "No data available."]
...
<table width=100% align="center" style="font-size:11px">
    ...
    def send_report(self):
        if self._data is None or self._learner is None:
            return
        self.report_items(_tr.m[1928, "Settings"], [(_tr.m[1929, "Permutations"], self.n_permutations)])
        self.report_raw(_tr.m[1930, "Info"], self._info.text())
        self.report_name(_tr.m[1931, "Plot"])
        self.report_plot()
