...
class OWConfusionMatrix(widget.OWWidget):
    ...
    name = _tr.m[1797, "Confusion Matrix"]
    description = (_tr.m[1798, "Display a confusion matrix constructed from "] + _tr.m[1799, "the results of classifier evaluations."])
    icon = "icons/ConfusionMatrix.svg"
    priority = 1001
    keywords = _tr.m[1800, "confusion matrix"]

    class Inputs:
        evaluation_results = Input(_tr.m[1801, "Evaluation Results"], Orange.evaluation.Results, id="Evaluation Results")

    class Outputs:
        selected_data = Output(_tr.m[1802, "Selected Data"], Orange.data.Table, id="Selected Data", default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Orange.data.Table, id="Data")

    quantities = [_tr.m[1803, "Number of instances"],
                  _tr.m[1804, "Proportion of predicted"],
                  _tr.m[1805, "Proportion of actual"],
                  _tr.m[1806, "Sum of probabilities"]]
    qu_tooltips = [_tr.m[1807, "Number of correctly and incorrectly classified instances"],
                   _tr.m[1808, "Proportion of predicted"],
                   _tr.m[1809, "Proportion of actual"],
                   (_tr.m[1810, "Number of instances, distributed across columns "] + _tr.m[1811, "according to predicted probabilities"])]
    ...
    UserAdviceMessages = [
        widget.Message(
            (_tr.m[1812, "Clicking on cells or in headers outputs the corresponding "] + _tr.m[1813, "data instances"]),
            "click_cell")]

    class Error(widget.OWWidget.Error):
        no_regression = Msg(_tr.m[1814, "Confusion Matrix cannot show regression results."])
        invalid_values = Msg(_tr.m[1815, "Evaluation Results input contains invalid values"])
        empty_input = widget.Msg(_tr.m[1816, "Empty result on input. Nothing to display."])

    def __init__(self):
        super().__init__()

        # ── wiget_card_26_work.md §2 / windowwk.md 유형 B 패턴 ────────────────
        # 가변 크기 위젯 — Qt.Window(NORMAL) → Qt.Dialog(DIALOG) 타입으로 변경.
        ...
        self.learners_box = gui.listBox(
            self.controlArea, self, "selected_learner", "learners", box=_tr.m[1817, 'Learners'],
            callback=self._learner_changed
        )

        self.outputbox = gui.vBox(self.buttonsArea)
        box = gui.vBox(self.outputbox, box=_tr.m[1818, "Output"])
        gui.checkBox(box, self, "append_predictions",
                     _tr.m[1819, "Predictions"], callback=self._invalidate)
        gui.checkBox(box, self, "append_probabilities",
                     _tr.m[1820, "Probabilities"],
        ...
        sbox = gui.hBox(box)
        gui.rubber(sbox)
        gui.comboBox(sbox, self, "selected_quantity", label=_tr.m[1821, "Show: "],
        ...
        selbox = gui.hBox(box)
        gui.button(selbox, self, _tr.m[1822, "Select Correct"],
                   callback=self.select_correct, autoDefault=False)
        gui.button(selbox, self, _tr.m[1823, "Select Misclassified"],
                   callback=self.select_wrong, autoDefault=False)
        gui.button(selbox, self, _tr.m[1824, "Clear Selection"],
    ...
    def _init_table(self, nclasses):
        item = self._item(0, 2)
        item.setData(_tr.m[1825, "Predicted"], Qt.DisplayRole)
        item.setTextAlignment(Qt.AlignCenter)
        item.setFlags(Qt.NoItemFlags)

        self._set_item(0, 2, item)
        item = self._item(2, 0)
        item.setData(_tr.m[1826, "Actual"], Qt.DisplayRole)
    ...
    def set_results(self, results):
        ...
        self.learners = getattr(
            ...
            [_tr.e(_tr.c(1827, f"Learner #{i + 1}")) for i in range(nmodels)])
        ...
        if self.results is not None and self.selected_learner:
            ...
            for i in range(n):
                for j in range(n):
                    ...
                    item.setData(
                        _tr.m[1828, "NA"] if _isinvalid(val) else formatstr.format(val),
                        Qt.DisplayRole)
                    ...
                    item.setToolTip(_tr.m[1829, "actual: {}\npredicted: {}"].format(
    ...
    def send_report(self):
        ...
        if self.results is not None and self.selected_learner:
            self.report_table(
                _tr.m[1830, "Confusion matrix for {} (showing {})"].
