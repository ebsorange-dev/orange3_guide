...
ERROR_OPTS = [_tr.m[1932, "(None)"], _tr.m[1933, "Difference"], _tr.m[1934, "Absolute difference"],
              _tr.m[1935, "Relative"], _tr.m[1936, "Absolute relative"]]
ERROR_TOOLTIPS = [
    _tr.m[1937, "Don't show columns with errors"],
    _tr.m[1938, "Show difference between predicted and actual value"],
    _tr.m[1939, "Show absolute difference between predicted and actual value"],
    _tr.m[1940, "Show relative difference between predicted and actual value"],
    _tr.m[1941, "Show absolute value of relative difference between predicted and actual value"]]


class OWPredictions(OWWidget):
    name = _tr.m[1942, "Predictions"]
    icon = "icons/Predictions.svg"
    priority = 200
    description = _tr.m[1943, "Display predictions of models for an input dataset."]
    keywords = _tr.m[1944, "predictions"]
    ...
    class Inputs:
        data = Input(_tr.m[1945, "Data"], Orange.data.Table, id="Data")
        predictors = MultiInput(_tr.m[1946, "Predictors"], Model, id="Predictors", filter_none=True)

    class Outputs:
        selected_predictions = Output(_tr.m[1947, "Selected Predictions"], Orange.data.Table, id="Selected Predictions",
                                      default=True, replaces=["Predictions"])
        annotated = Output(_tr.m[1949, "Predictions"], Orange.data.Table, id="Predictions")
        evaluation_results = Output(_tr.m[1950, "Evaluation Results"], Results, id="Evaluation Results")

    class Warning(OWWidget.Warning):
        empty_data = Msg(_tr.m[1951, "Empty dataset"])
        wrong_targets = Msg(
            _tr.m[1952, "Some model(s) predict a different target (see more ...)\n{}"])
        missing_targets = Msg((_tr.m[1953, "Instances with missing targets "] + _tr.m[1954, "are ignored while scoring."]))

    class Error(OWWidget.Error):
        predictor_failed = Msg(_tr.m[1955, "Some predictor(s) failed (see more ...)\n{}"])
        scorer_failed = Msg(_tr.m[1956, "Some scorer(s) failed (see more ...)\n{}"])

    settingsHandler = settings.ClassValuesContextHandler()
    score_table = settings.SettingProvider(ScoreTable)

    #: List of selected class value indices in the `class_values` list
    PROB_OPTS = [_tr.m[1957, "(None)"],
                 _tr.m[1958, "Classes in data"], _tr.m[1959, "Classes known to the model"], _tr.m[1960, "Classes in data and model"]]
    PROB_TOOLTIPS = [_tr.m[1961, "Don't show probabilities"],
                     _tr.m[1962, "Show probabilities for classes in the data"],
                     (_tr.m[1963, "Show probabilities for classes known to the model,\n"] + _tr.m[1964, "including those that don't appear in this data"]),
                     (_tr.m[1965, "Show probabilities for classes in data that are also\n"] + _tr.m[1966, "known to the model"])
                     ]
    ...
    TARGET_AVERAGE = _tr.m[1967, "(Average over classes)"]
    ...
    def __init__(self):
        ...
        self._prob_controls = [
            gui.widgetLabel(predopts, _tr.m[1968, "Show probabilities for"]),
            gui.comboBox(
        ...
        self._cls_error_controls = [
            gui.checkBox(
                ...
                _tr.m[1969, "Show classification errors"],
                tooltip=_tr.m[1970, "Show 1 - probability assigned to the correct class"],
                callback=self._update_errors_visibility
            )
        ]

        err_label = gui.widgetLabel(predopts, _tr.m[1971, "Shown regression error: "])
        err_combo = gui.comboBox(
            predopts, self, "show_reg_errors", items=ERROR_OPTS,
            callback=self._reg_error_changed,
            toolTip=_tr.m[1972, "See tooltips for individual options"])
        ...
        gui.rubber(predopts)
        self.reset_button = button = QPushButton(_tr.m[1973, "Restore Original Order"])
        button.clicked.connect(self._reset_order)
        button.setToolTip(_tr.m[1974, "Show rows in the original order"])
        ...
        self.score_opt_box = scoreopts = gui.hBox(
            None, sizePolicy=(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed))
        gui.checkBox(
            scoreopts, self, "show_scores", _tr.m[1975, "Show perfomance scores"],
            callback=self._update_score_table_visibility
        )
        gui.separator(scoreopts, 32)
        self._target_controls = [
            gui.widgetLabel(scoreopts, _tr.m[1976, "Target class:"]),
            gui.comboBox(
    ...
    def _update_scores(self):
        ...
        for pred in self.predictors:
            ...
            row = [QStandardItem(learner_name(pred.predictor)),
                   QStandardItem(_tr.m[1977, "N/A"]), QStandardItem(_tr.m[1978, "N/A"])]
    ...
    def _get_details(self):
        ...
        if n_predictors:
            ...
            details += _tr.e(_tr.c(1981, f"Model: {n_predictors} {pl(n_predictors, 'model')}"))
            if n_valid != n_predictors:
                details += _tr.e(_tr.c(1982, f" ({n_predictors - n_valid} failed)"))
            details += "<ul>"
            for name in pred_names:
                details += f"<li>{name}</li>"
            details += "</ul>"
        else:
            details += _tr.m[1983, "Model:<br>No model on input."]
        return details
    ...
    def _add_error_out_columns(self, slot, newmetas, newcolumns, index):
        if self.shown_errors:
            name = _tr.e(_tr.c(1984, f"{slot.predictor.name} (error)"))
    ...
    def send_report(self):
        ...
        if self.data:
            ...
            if self.is_discrete_class and self.shown_probs != self.NO_PROBS:
                text += _tr.m[1985, '<br>Showing probabilities for ']
                if self.shown_probs == self.MODEL_PROBS:
                    text += _tr.m[1986, "all classes known to the model."]
                elif self.shown_probs == self.DATA_PROBS:
                    text += _tr.m[1987, "all classes that appear in the data."]
                elif self.shown_probs == self.BOTH_PROBS:
                    text += (_tr.m[1988, "all classes that appear in the data "] + _tr.m[1989, "and are known to the model."])
                else:
                    class_idx = self.shown_probs - len(self.PROB_OPTS)
                    text += f"'{self.class_var.values[class_idx]}.'"
            self.report_paragraph('Info', text)
            self.report_table(_tr.m[1990, "Data & Predictions"], merge_data_with_predictions(),
                              header_rows=1, header_columns=1)

            self.report_name(_tr.m[1991, "Scores"])
            if self.is_discrete_class:
                self.report_items([(_tr.m[1992, "Target class"], self.target_class)])
