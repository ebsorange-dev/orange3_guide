...
class OWROCAnalysis(widget.OWWidget):
    name = _tr.m[1993, "ROC Analysis"]
    description = (_tr.m[1994, "Display the Receiver Operating Characteristics curve "] + _tr.m[1995, "based on the evaluation of classifiers."])
    icon = "icons/ROCAnalysis.svg"
    priority = 1010
    keywords = _tr.m[1996, "roc analysis, analyse"]

    class Inputs:
        evaluation_results = Input(_tr.m[1997, "Evaluation Results"], Orange.evaluation.Results, id="Evaluation Results")

    class Outputs:
        calibrated_model = Output(_tr.m[1998, "Calibrated Model"], Model, id="Calibrated Model")

    class Information(widget.OWWidget.Information):
        no_output = Msg(_tr.m[1999, "Can't output a model: {}"])
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2000, "Plot"])
        self.target_cb = gui.comboBox(
            box, self, "target_index",
            label=_tr.m[2001, "Target"], orientation=Qt.Horizontal,
            callback=self._on_target_changed,
            contentsLength=8, searchable=True)

        gui.widgetLabel(box, _tr.m[2002, "Classifiers"])
        ...
        abox = gui.vBox(self.controlArea, _tr.m[2003, "Curves"])
        gui.comboBox(abox, self, "roc_averaging",
                     items=[_tr.m[2004, "Merge Predictions from Folds"], _tr.m[2005, "Mean TP Rate"],
                            _tr.m[2006, "Mean TP and FP at Threshold"], _tr.m[2007, "Show Individual Curves"]],
                     callback=self._replot)

        gui.checkBox(abox, self, "display_convex_curve",
                     _tr.m[2008, "Show convex ROC curves"], callback=self._replot)
        gui.checkBox(abox, self, "display_convex_hull",
                     _tr.m[2009, "Show ROC convex hull"], callback=self._replot)

        box = gui.vBox(self.controlArea, _tr.m[2010, "Analysis"])

        gui.checkBox(box, self, "display_def_threshold",
                     _tr.m[2011, "Default threshold (0.5) point"],
                     callback=self._on_display_def_threshold_changed)

        gui.checkBox(box, self, "display_perf_line", _tr.m[2012, "Show performance line"],
        ...
        sp = gui.spin(box, self, "fp_cost", 1, 1000, 10,
                      ...
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[2013, "FP Cost:"]), 0, 0)
        grid.addWidget(sp, 0, 1)

        sp = gui.spin(box, self, "fn_cost", 1, 1000, 10,
                      alignment=Qt.AlignRight,
                      callback=self._on_display_perf_line_changed)
        grid.addWidget(QLabel(_tr.m[2014, "FN Cost:"]))
        ...
        grid.addWidget(QLabel(_tr.m[2015, "Prior probability:"]))
        ...
        axis.setLabel(_tr.m[2016, "FP Rate (1-Specificity)"])
        axis.setGrid(16)

        axis = self.plot.getAxis("left")
        axis.setTickFont(tickfont)
        axis.setLabel(_tr.m[2017, "TP Rate (Sensitivity)"])
        axis.setGrid(16)
    ...
    def _setup_plot(self):
        ...
        if not all(c.is_valid for c in hull_curves):
            if any(c.is_valid for c in hull_curves):
                warning = _tr.m[2018, "Some ROC curves are undefined"]
            else:
                warning = _tr.m[2019, "All ROC curves are undefined"]
    ...
    def send_report(self):
        if self.results is None:
            return
        items = OrderedDict()
        items[_tr.m[2021, "Target class"]] = self.target_cb.currentText()
        if self.display_perf_line:
            items[_tr.m[2022, "Costs"]] = \
                _tr.m[2023, "FP = {}, FN = {}"].format(self.fp_cost, self.fn_cost)
            items[_tr.m[2024, "Target probability"]] = "{} %".format(self.target_prior)
