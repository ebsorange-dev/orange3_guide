...
class OWGenialisExpressions(widget.OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        # 2026-05-28: Genialis API (app.genialis.com) 가 400 Bad Request 응답하는
        # 경우(서버 측 API 스펙 변경 또는 네트워크 문제) 위젯 자체가 로딩 실패하던
        # 문제 회피 — silent sign-in 실패는 무시하고 위젯은 정상 표시.
        # 사용자가 수동으로 Sign In 버튼 클릭 시 정상 sign_in 흐름 진행.
        try:
            self.sign_in(silent=True)
        except Exception as _e:
            import logging as _logging
            _logging.getLogger(__name__).warning(
                "Genialis silent sign-in 실패 (무시): %s", _e
            )
    ...
    def res(self, value: resolwe.resapi.ResolweAPI):
        if isinstance(value, resolwe.resapi.ResolweAPI):
            ...
            # 2026-05-28: Genialis API 호출이 400 등으로 실패해도 widget 은 살아있도록
            # try/except 로 보호. 사용자에게는 빈 collection list 가 표시됨.
            try:
                self.update_collections_view()
            except Exception as _e:
                import logging as _logging
                _logging.getLogger(__name__).warning(
                    "Genialis update_collections_view 실패 (무시): %s", _e
                )
            self.__invalidate()
