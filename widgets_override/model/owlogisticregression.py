...
class OWLogisticRegression(OWBaseLearner):
    name = _tr.m[2262, "Logistic Regression"]
    description = (_tr.m[2263, "The logistic regression classification algorithm with "] + _tr.m[2264, "LASSO (L1) or ridge (L2) regularization."])
    icon = "icons/LogisticRegression.svg"
    replaces = [
        "Orange.widgets.classify.owlogisticregression.OWLogisticRegression",
    ]
    priority = 60
    keywords = _tr.m[2265, "logistic regression"]

    LEARNER = LogisticRegressionLearner

    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output(_tr.m[2266, "Coefficients"], Table, id="Coefficients", explicit=True)
    ...
    penalty_types = ("Lasso (L1)", "Ridge (L2)", _tr.m[2267, "None"])
    penalty_types_short = ["l1", "l2", None]

    class Warning(OWBaseLearner.Warning):
        class_weights_used = Msg(_tr.m[2268, "Weighting by class may decrease performance."])

    def add_main_layout(self):
        # this is part of init, pylint: disable=attribute-defined-outside-init
        box = gui.widgetBox(self.controlArea, box=True)
        self.penalty_combo = gui.comboBox(
            box, self, "penalty_type", label=_tr.m[2269, "Regularization type: "],
            items=self.penalty_types, orientation=Qt.Horizontal,
            callback=self._penalty_type_changed)
        self.c_box = box = gui.widgetBox(box)
        gui.widgetLabel(box, _tr.m[2270, "Strength:"])
        box2 = gui.hBox(gui.indentedBox(box))
        gui.widgetLabel(box2, _tr.m[2271, "Weak"]).setStyleSheet("margin-top:6px")
        self.c_slider = gui.hSlider(
            ...
            createLabel=False)
        gui.widgetLabel(box2, _tr.m[2272, "Strong"]).setStyleSheet("margin-top:6px")
        box2 = gui.hBox(box)
        ...
        self.weights = gui.checkBox(
            box, self,
            "class_weight", label=_tr.m[2273, "Balance class distribution"],
            callback=self.settings_changed,
            tooltip=_tr.m[2274, "Weigh classes inversely proportional to their frequencies."]
        )
    ...
    def set_c(self):
        ...
        if enable_c:
            ...
            self.c_label.setText(_tr.m[2275, "N/A"])
    ...
    def get_learner_parameters(self):
        return ((_tr.m[2276, "Regularization"], _tr.m[2277, "{}, C={}, class weights: {}"].format(
