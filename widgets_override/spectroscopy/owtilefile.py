...
class OWTilefile(widget.OWWidget, RecentPathsWComboMixin):
    ...
    def __init__(self):
        ...
        self._tilefile_poll_timer = QTimer(self)
        self._tilefile_poll_timer.setInterval(1500)
        self._tilefile_poll_timer.timeout.connect(self._poll_tilefile_upload)
        self._tilefile_poll_timer.start()
        ...
        # 2026-05-28: 위젯 더블클릭 직후 무조건 표시되던 file_too_big 경고 제거
        # ("The file is too large to load automatically. Press Reload to load.").
        # 실제 큰 파일 로딩 시도 시점에는 다른 진행 표시로 충분 — 초기 경고는 혼란 유발.
    ...
    def browse_file(self, in_demos=False):
        ...
        # 일반 모드: 브라우저 PC 파일 선택기 신호 파일 작성
        import time as _time
        try:
            with open("/config/.upload_request_spec_tilefile", "w") as _f:
                _f.write(str(_time.time()))
        except OSError:
            pass

    def _poll_tilefile_upload(self):
        """Tile File 단일 파일 업로드 완료 신호 폴링."""
        sig = "/config/.upload_path_spec_tilefile"
