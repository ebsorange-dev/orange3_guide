...
class OWMultifile(widget.OWWidget, RelocatablePathsWidgetMixin):
    ...
    def __init__(self):
        ...
        # 브라우저 PC 파일 선택기 폴링 (2026-05-28)
        self._multifile_poll_timer = QTimer(self)
        self._multifile_poll_timer.setInterval(1500)
        self._multifile_poll_timer.timeout.connect(self._poll_multifile_uploads)
        self._multifile_poll_timer.start()
    ...
    def browse_files(self):
        """Qt 다이얼로그 대신 브라우저 PC 파일 선택기 호출 (2026-05-28).
        세션매니저가 SSE 로 감지 → 브라우저 file input 클릭 → 업로드 완료 후
        /config/.upload_path_spec_multifile 신호 파일 → _poll_multifile_uploads() 적용."""
        import time as _time
        try:
            with open("/config/.upload_request_spec_multifile", "w") as _f:
                _f.write(str(_time.time()))
        except OSError:
            pass

    def _poll_multifile_uploads(self):
        """다중 파일 업로드 완료 신호 폴링 — newline-separated 경로 목록."""
        sig = "/config/.upload_path_spec_multifile"
        if not os.path.isfile(sig):
            return
        try:
            with open(sig) as _f:
                payload = _f.read().strip()
            os.remove(sig)
        except OSError:
            return
        if not payload:
            return
        paths = [p for p in payload.splitlines() if p.strip() and os.path.isfile(p.strip())]
        if not paths:
            return
        # reader=None → 자동 감지 (load_files 안에서 add_path 가 처리)
        self.load_files(paths, None)
