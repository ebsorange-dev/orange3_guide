...
class OWDataTable(OWWidget):
    category = _tr.m[2540, "Orange Obsolete"]
    # replaces 를 비워서 OWDataTable 매핑 우선권을 신규 OWTable 에 양보
    # (둘 다 같은 replaces 를 가지면 마지막 발견된 위젯이 선택되어 한글 모드에서
    #  obsolete 가 우선 매칭되는 문제 회피)
    replaces = []

    name = _tr.m[2541, "Data Table"]
    description = _tr.m[2542, "View the dataset in a spreadsheet."]
    icon = "../data/icons/Table.svg"
    priority = 50
    keywords = []

    class Inputs:
        data = MultiInput(_tr.m[2543, "Data"], Table, id="Data", auto_summary=False, filter_none=True)

    class Outputs:
        selected_data = Output(_tr.m[2544, "Selected Data"], Table, id="Selected Data", default=True)
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, id="Data")

    class Warning(OWWidget.Warning):
        multiple_inputs = Msg(
            (_tr.m[2545, "Multiple Data inputs are deprecated.\n"] + (_tr.m[2546, "This functionality will be removed soon.\n"] + _tr.m[2547, "Use multiple Tables instead."])))
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, _tr.m[2548, "Variables"])
        self.c_show_attribute_labels = gui.checkBox(
            box, self, "show_attribute_labels",
            _tr.m[2549, "Show variable labels (if present)"],
            callback=self._on_show_variable_labels_changed)

        gui.checkBox(box, self, "show_distributions",
                     _tr.m[2550, 'Visualize numeric values'],
                     callback=self._on_distribution_color_changed)
        gui.checkBox(box, self, "color_by_class", _tr.m[2551, 'Color by instance classes'],
                     callback=self._on_distribution_color_changed)

        box = gui.vBox(self.controlArea, _tr.m[2552, "Selection"])

        gui.checkBox(box, self, "select_rows", _tr.m[2553, "Select full rows"],
                     callback=self._on_select_rows_changed)

        gui.rubber(self.controlArea)

        gui.button(self.buttonsArea, self, _tr.m[2554, "Restore Original Order"],
                   callback=self.restore_order,
                   tooltip=_tr.m[2555, "Show rows in the original order"],
                   autoDefault=False,
    ...
    def set_dataset(self, index: int, data: Table):
        ...
        datasetname = getattr(data, "name", _tr.m[2556, "Data"])
    ...
    def insert_dataset(self, index: int, data: Table):
        datasetname = getattr(data, "name", _tr.m[2557, "Data"])
