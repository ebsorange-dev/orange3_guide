from orangecontrib.timeseries.widgets._ko_trans import _t  # i18n (2026-05-21)
from collections import Counter
from itertools import chain

import numpy as np

from AnyQt.QtCore import Qt, QSortFilterProxyModel, QRect, QSize
...
class OWMovingTransform(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        no_aggregations = widget.Msg(_t("No (applicable) aggregations are selected"))
        inapplicable_aggregations = \
            widget.Msg("Some aggregations are applicable "
                       "only to sliding window ({})")
        window_to_large = widget.Msg(_t("Window width is too large"))
        block_to_large = widget.Msg(_t("Block width is too large"))
    ...
    REF_OPTIONS = (_t("Discard original data"), _t("Keep first instance"),
                   _t("Keep middle instance"), _t("Keep last instance"))

    KeepComplete, KeepAll = range(1, 3)
    KEEP_OPTIONS = (_t("Discard original data"),
                    _t("Keep original data"),
                    _t("Include leading instances"))
    KEEP_TOOLTIPS = (_t("Output data will contain only aggregates."),
    ...
    period_width = settings.Setting(_t("Years"))
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(buttons, _t("Consecutive blocks"), buttons)
        indbox = gui.indentedBox(buttons)
        gui.spin(
            indbox, self, 'block_width',
            2, 1000, label=_t('Block width:'),
            controlWidth=80, alignment=Qt.AlignRight,
        ...
        self.rb_period = gui.appendRadioButton(
            buttons, _t("Aggregate time periods"), buttons)
        indbox = gui.indentedBox(buttons)
        cb = gui.comboBox(
        ...
        self.filter_line = QLineEdit(placeholderText=_t("Filter ..."))
        ...
        gui.checkBox(vbox, self, "only_numeric", _t("Show only numeric variables"),
        ...
        gui.auto_commit(self.buttonsArea, self, 'autocommit', _t('&Apply'))
    ...
    def _names_for_blocked_aggregation(self):
        ...
        if self.method == self.TimePeriods:
            names += [self.period_width, _t("Instance count")]
        names += [f"{var.name} ({trans})"
                  for i, var in enumerate(model)
                  for trans in model.get_transformations(i)
    ...
    def send_report(self):
        if self.method == self.SlidingWindow:
            self.report_items(
                _t("Sliding Window"),
                ((_t("Window width"), self.window_width),
                 (_t("Original data"), self.KEEP_OPTIONS[self.keep_instances].lower())))
        elif self.method == self.SequentialBlocks:
            self.report_items(
                _t("Consecutive blocks"),
                ((_t("Block width"), self.block_width),
                 (_t("Original data"), self.REF_OPTIONS[self.ref_instance].lower())))
        else:
            assert self.method == self.TimePeriods
            self.report_items(
                _t("Aggregate time periods"),
                ((_t("Period"), self.period_width), )
            )
        ...
        if transformations:
            self.report_items(_t("Transformations"), tuple(transformations))
