...
class OWNxExplorer(OWDataProjectionWidget, ConcurrentWidgetMixin):
    name = _tr.m[63, "Network Explorer"]
    description = _tr.m[64, "Visually explore the network and its properties."]
    icon = "icons/NetworkExplorer.svg"
    priority = 6420

    class Inputs:
        node_data = Input(_tr.m[65, "Node Data"], Table, id="Node Data")
        node_subset = Input(_tr.m[66, "Node Subset"], Table, id="Node Subset")
        network = Input(_tr.m[67, "Network"], Network, id="Network", default=True)
        node_distances = Input(_tr.m[68, "Node Distances"], Orange.misc.DistMatrix, id="Node Distances")

    class Outputs(OWDataProjectionWidget.Outputs):
        subgraph = Output(_tr.m[69, "Selected sub-network"], Network, id="Selected sub-network")
        unselected_subgraph = Output(_tr.m[70, "Remaining sub-network"], Network, id="Remaining sub-network")
        distances = Output(_tr.m[71, "Distance matrix"], Orange.misc.DistMatrix, id="Distance matrix")

    UserAdviceMessages = [
        Message(_tr.m[72, "Double clicks select connected components"],
                Message.Information),
    ]
    ...
    class Warning(OWDataProjectionWidget.Warning):
        distance_matrix_mismatch = Msg(
            (_tr.m[73, "Distance matrix size doesn't match the number of network nodes "] + _tr.m[74, "and will be ignored."]))
        no_graph_found = Msg(_tr.m[75, "Node data is given, graph data is missing."])

    class Error(OWDataProjectionWidget.Error):
        data_size_mismatch = Msg(
            _tr.m[76, "Length of the data does not match the number of nodes."])
        network_too_large = Msg(_tr.m[77, "Network is too large to visualize."])
    ...
    def __init__(self):
        ...
        # ── wiget_card_26_work.md §2 / windowwk.md 유형 B 패턴 ────────────────
        # 가변 크기 시각화 위젯 — Qt.Window(NORMAL) → Qt.Dialog(DIALOG) 타입으로 변경.
        # Openbox 의 <application type="normal"><maximized>true</maximized> 자동 최대화
        # 회피 + 사용자가 마우스로 창 크기 조절 가능 + 최대화 버튼 유지.
        # 카드 chrome (_apply_clean_chrome) 가 windowFlags() 보존하며
        # FramelessWindowHint 만 추가하므로 본 설정과 충돌 없음.
        self.setWindowFlags(
    ...
    def _add_info_box(self):
        # pylint: disable=attribute-defined-outside-init
        info = gui.vBox(self.controlArea, box=_tr.m[78, "Layout"])
        gui.label(
            info, self,
            (_tr.m[79, "Nodes: %(number_of_nodes)i (%(nodes_per_edge).2f per edge); "] + _tr.m[80, "%(nSelected)i selected"]))
        gui.label(
            info, self,
            _tr.m[81, "Edges: %(number_of_edges)i (%(edges_per_node).2f per node)"])
        lbox = gui.hBox(info)
        self.relayout_button = gui.button(
            lbox, self, _tr.m[82, 'Improve'], callback=self.improve, autoDefault=False,
            tooltip=_tr.m[83, "Optimize the current layout, with a small initial jerk"])
        self.stop_button = gui.button(
            lbox, self, _tr.m[84, 'Stop'], callback=self.stop_relayout, autoDefault=False,
            hidden=True)
        self.randomize_button = gui.button(
            lbox, self, _tr.m[85, 'Re-layout'], callback=self.restart, autoDefault=False,
            tooltip=_tr.m[86, "Restart laying out from random positions"])
        gui.hSlider(info, self, "layout_density", minValue=1, maxValue=50,
                    label=_tr.m[87, "Gravity"], orientation=Qt.Horizontal,
                    callback_finished=self.improve,
                    tooltip=(_tr.m[88, "Lower values improve optimization,\n"] + (_tr.m[89, "higher work better for graph with many small "] + _tr.m[90, "components"])))
        gui.checkBox(info, self, "observe_weights",
                     label=_tr.m[91, "Make edges with large weights shorter"],
                     callback=self.improve)

    def _add_effects_box(self):
        # pylint: disable=attribute-defined-outside-init
        gbox = self.gui.create_gridbox(self.controlArea, box=_tr.m[92, "Widths and Sizes"])
        self.gui.add_widget(self.gui.PointSize, gbox)
        gbox.layout().itemAtPosition(1, 0).widget().setText(_tr.m[93, "Node Size:"])
        self.gui.add_control(
            gbox, gui.hSlider, _tr.m[94, "Edge width:"],
    ...
    def _add_mark_box(self):
        ...
        def text_line():
            ...
            return gui.lineEdit(
                gui.hBox(vbox), self, "mark_text", label=_tr.m[98, "Text: "],
        ...
        self.mark_criteria = [
            (_tr.m[99, "(Select criteria for marking)"], None, lambda: np.zeros((0,))),
            (_tr.m[100, "Mark node labelled as"], text_line(), mark_label_equals),
            (_tr.m[101, "Mark nodes whose label starts with"], text_line(), mark_label_starts),
            (_tr.m[102, "Mark nodes whose label contains"], text_line(), mark_label_contains),
            (_tr.m[103, "Mark nodes whose data contains"], text_line(), mark_text),
            (_tr.m[104, "Mark nodes reachable from selected"], None, mark_reachable),

            (_tr.m[105, "Mark nodes in vicinity of selection"],
             spin("mark_hops", _tr.m[106, "Number of hops:"], 1, 20),
             mark_close),

            (_tr.m[107, "Mark nodes from subset signal"], None, mark_from_input),

            (_tr.m[108, "Mark nodes with few connections"],
             spin("mark_max_conn", _tr.m[109, "Max. connections:"], 0, 1000),
             lambda: np.flatnonzero(self.network.degrees() <= self.mark_max_conn)),

            (_tr.m[110, "Mark nodes with many connections"],
             spin("mark_min_conn", _tr.m[111, "Min. connections:"], 1, 1000),
             lambda: np.flatnonzero(self.network.degrees() >= self.mark_min_conn)),

            (_tr.m[112, "Mark nodes with most connections"],
             spin("mark_most_conn", _tr.m[113, "Number of marked:"], 1, 1000),
             mark_most_connections),

            (_tr.m[114, "Mark nodes with more connections than any neighbour"], None,
             mark_more_than_any_neighbour),

            (_tr.m[115, "Mark nodes with more connections than average neighbour"], None,
        ...
        self.btselect = gui.button(
            hbox, self, _tr.m[116, "Select"], callback=self.select_marked)
        self.btadd = gui.button(
            hbox, self, _tr.m[117, "Add to Selection"], callback=self.select_add_marked)
        self.btgroup = gui.button(
            hbox, self, _tr.m[118, "Add New Group"], callback=self.select_as_group)
    ...
    def update_selection_buttons(self):
        ...
        if len(selection) == 0 or np.max(selection) == 0:
            ...
            self.btadd.setText(_tr.m[119, "Add to Selection"])
            self.btadd.show()
            self.btgroup.hide()
        else:
            self.btadd.setText(_tr.m[120, "Add to Group"])
            self.btadd.show()
            self.btgroup.show()
            ...
            if self.node_data is None:
                if isinstance(network.nodes, Table):
                    ...
                    self.data = Table.from_numpy(
                        Domain([], None, [StringVariable(_tr.m[121, "label"])]),
    ...
    def send_report(self):
        ...
        self.report_items(_tr.m[122, 'Graph info'], [
            (_tr.m[123, "Number of vertices"], self.network.number_of_nodes()),
            (_tr.m[124, "Number of edges"], self.network.number_of_edges()),
            (_tr.m[125, "Vertices per edge"], round(self.nodes_per_edge, 3)),
            (_tr.m[126, "Edges per vertex"], round(self.edges_per_node, 3)),
        ])
        if self.data is not None:
            self.report_data(_tr.m[127, "Data"], self.data)
        if any((self.attr_color, self.attr_shape,
                self.attr_size, self.attr_label)):
            self.report_items(
                _tr.m[128, "Visual settings"],
                [(_tr.m[129, "Color"], self._get_caption_var_name(self.attr_color)),
                 (_tr.m[130, "Label"], self._get_caption_var_name(self.attr_label)),
                 (_tr.m[131, "Shape"], self._get_caption_var_name(self.attr_shape)),
                 (_tr.m[132, "Size"], self._get_caption_var_name(self.attr_size))])
        self.report_plot()
