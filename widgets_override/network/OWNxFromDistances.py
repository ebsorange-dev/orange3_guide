...
class OWNxFromDistances(widget.OWWidget):
    name = _tr.m[160, "Network From Distances"]
    description = (_tr.m[161, 'Constructs a network by connecting nodes with distances '] + _tr.m[162, 'below some threshold.'])
    icon = "icons/NetworkFromDistances.svg"
    priority = 6440

    class Inputs:
        distances = Input(_tr.m[163, "Distances"], DistMatrix, id="Distances")

    class Outputs:
        network = Output(_tr.m[164, "Network"], Network, id="Network")
    ...
    class Warning(widget.OWWidget.Warning):
        large_number_of_nodes = \
            Msg(_tr.m[165, 'Large number of nodes/edges; expect slow performance.'])

    class Error(widget.OWWidget.Error):
        number_of_edges = Msg(_tr.m[166, 'The network is too large ({})'])
    ...
    def __init__(self):
        ...
        gui.widgetLabel(hbox, _tr.m[167, "Threshold:"])
        ...
        gui.widgetLabel(hbox, _tr.m[168, "Number of edges:"])
        ...
        gui.widgetLabel(hbox, _tr.m[169, "Density (%):"])
    ...
    def send_report(self):
        ...
        self.report_items(_tr.m[170, "Settings"], [
            (_tr.m[171, "Threshold"], self.threshold_edit.text()),
            (_tr.m[172, "Density"], self.density_edit.text()),
            (_tr.m[173, "Edges"], self.edges_edit.text()),
        ])

        self.report_name(_tr.m[174, "Histogram"])
        self.report_plot(self.histogram)

        nodes, edges = self.graph_stat
        self.report_items(
            _tr.m[175, "Output network"],
            [(_tr.m[176, "Vertices"], len(self.matrix)),
             (_tr.m[177, "Edges"], (f"{edges} " + (_tr.e(_tr.c(178, f"({edges / nodes:.2f} per vertex), ")) + _tr.e(_tr.c(179, f"density: {edges / self.eff_distances:.2%}")))))])
...
class Histogram(pg.PlotWidget):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # Set axes titles
        self.plotItem.setLabel('bottom', _tr.m[180, "Threshold Distance"])
        self.plotItem.setLabel('left', _tr.m[181, "Number of Edges"])
        prop_axis.setLabel(_tr.m[182, "Edge Density"], units="%")
