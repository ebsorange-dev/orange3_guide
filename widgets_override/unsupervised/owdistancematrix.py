...
class OWDistanceMatrix(widget.OWWidget):
    name = _tr.m[2676, "Distance Matrix"]
    description = _tr.m[2677, "View distance matrix."]
    icon = "icons/DistanceMatrix.svg"
    priority = 200
    keywords = _tr.m[2678, "distance matrix"]

    class Inputs:
        distances = Input(_tr.m[2679, "Distances"], DistMatrix, id="Distances")

    class Outputs:
        distances = Output(_tr.m[2680, "Distances"], DistMatrix, id="Distances", dynamic=False)
        table = Output(_tr.m[2681, "Selected Data"], Table, id="Selected Data", replaces=["Table"])

    class Error(widget.OWWidget.Error):
        empty_matrix = widget.Msg(_tr.m[2683, "Distance matrix is empty."])
    ...
    def __init__(self):
        ...
        self.annot_combo = gui.comboBox(
            self.buttonsArea, self, "annotation_idx", label=_tr.m[2684, "Labels: "],
            orientation=Qt.Horizontal,
            callback=self._invalidate_annotations, contentsLength=12)
        self.annot_combo.setModel(VariableListModel())
        self.annot_combo.model()[:] = [_tr.m[2685, "None"], _tr.m[2686, "Enumeration"]]
    ...
    def set_distances(self, distances):
        ...
        annotations = [_tr.m[2687, "None"], _tr.m[2688, "Enumerate"]]
        view = self.tableview
        ...
        if distances is not None:
            ...
            if not distances.is_symmetric():
                ...
                if distances.row_items is not None \
                    ...
                    annotations.append(_tr.m[2689, "Labels"])
                    pending_idx = 2
            else:
                ...
                if items and not distances.axis:
                    annotations.append(_tr.m[2690, "Attribute names"])
                    pending_idx = 2
                elif isinstance(items, list) and \
                        all(isinstance(item, Variable) for item in items):
                    annotations.append(_tr.m[2691, "Name"])
                    pending_idx = 2
    ...
    def _choose_label(data: Table):
        attr = max((attr for attr in data.domain.metas
                   ...
                   default=None)
        return attr or data.domain.class_var or _tr.m[2692, "Enumerate"]
    ...
    def _update_labels(self):
        ...
        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2693, "Attribute names"]:
            attr = self.distances.row_items.domain.attributes
            ver_labels = hor_labels = [
                str(attr[i]) for i in range(self.distances.shape[0])]

        elif self.annot_combo.model()[self.annotation_idx] == _tr.m[2694, "Labels"]:
...
if __name__ == "__main__":  # pragma: no cover
    ...
    data = Orange.data.Table(_tr.m[2695, "zoo"])
