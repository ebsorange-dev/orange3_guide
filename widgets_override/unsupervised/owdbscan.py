...
class OWDBSCAN(widget.OWWidget):
    name = "DBSCAN"
    description = _tr.m[2603, "Density-based spatial clustering."]
    icon = "icons/DBSCAN.svg"
    priority = 2150

    class Inputs:
        data = Input(_tr.m[2604, "Data"], Table, id="Data")

    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, id="Data")

    class Error(widget.OWWidget.Error):
        not_enough_instances = Msg((_tr.m[2605, "Not enough unique data instances. "] + _tr.m[2606, "At least two are required."]))

    METRICS = [
        (_tr.m[2607, "Euclidean"], "euclidean"),
        (_tr.m[2608, "Manhattan"], "cityblock"),
        (_tr.m[2609, "Cosine"], "cosine")
    ]
    ...
    def __init__(self):
        ...
        box = gui.widgetBox(self.controlArea, _tr.m[2610, "Parameters"])
        gui.spin(box, self, "min_samples", 1, 100, 1,
                 callback=self._min_samples_changed,
                 label=_tr.m[2611, "Core point neighbors"])
        gui.doubleSpin(box, self, "eps", EPS_BOTTOM_LIMIT, 1000, 0.01,
                       callback=self._eps_changed,
                       label=_tr.m[2612, "Neighborhood distance"])

        box = gui.widgetBox(self.controlArea, self.tr(_tr.m[2613, "Distance Metric"]))
        gui.comboBox(box, self, "metric_idx",
                     items=list(zip(*self.METRICS))[0],
                     callback=self._metirc_changed)
        gui.checkBox(box, self, "normalize", _tr.m[2614, "Normalize features"],
        ...
        self.plot = SliderGraph(
            x_axis_label=_tr.m[2615, "Data items sorted by score"],
            y_axis_label=_tr.m[2616, "Distance to the k-th nearest neighbour"],
    ...
    def send_data(self):
        ...
        u_clust_var = get_unique_names(names, _tr.m[2617, "Cluster"])
        clust_var = DiscreteVariable(
            u_clust_var, values=[_tr.m[2618, "C%d"] % (x + 1) for x in range(k)])

        u_in_core = get_unique_names(names + [u_clust_var], _tr.m[2619, "DBSCAN Core"])
