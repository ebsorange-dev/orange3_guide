...
class ClusterTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            return "{:.3f}".format(score) if valid else _tr.m[2823, "NA"]
...
class OWKMeans(widget.OWWidget):
    name = _tr.m[2824, "k-Means"]
    description = (_tr.m[2825, "k-Means clustering algorithm with silhouette-based "] + _tr.m[2826, "quality estimation."])
    icon = "icons/KMeans.svg"
    priority = 2100
    keywords = _tr.m[2827, "k-means, kmeans, clustering"]

    class Inputs:
        data = Input(_tr.m[2828, "Data"], Table, id="Data")

    class Outputs:
        annotated_data = Output(
            ANNOTATED_DATA_SIGNAL_NAME, Table, default=True,
            id="Data", replaces=["Annotated Data"]
        )
        centroids = Output(_tr.m[2830, "Centroids"], Table, id="Centroids", dynamic=False)

    class Error(widget.OWWidget.Error):
        failed = widget.Msg(_tr.m[2831, "Clustering failed\nError: {}"])
        not_enough_data = widget.Msg(
            _tr.m[2832, "Too few ({}) unique data instances for {} clusters"]
        )
        no_attributes = widget.Msg(_tr.m[2833, "Data is missing features."])

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg(
            _tr.m[2834, "Silhouette scores are not computed for >{} samples"].format(
                SILHOUETTE_MAX_SAMPLES)
        )
        not_enough_data = widget.Msg(
            _tr.m[2835, "Too few ({}) unique data instances for {} clusters"]
        )
        no_sparse_normalization = widget.Msg(_tr.m[2836, "Sparse data cannot be normalized"])

    INIT_METHODS = ((_tr.m[2837, "Initialize with KMeans++"], "k-means++"),
                    (_tr.m[2838, "Random initialization"], "random"))
    ...
    def __init__(self):
        ...
        bg = gui.radioButtonsInBox(
            ...
            box=_tr.m[2839, "Number of Clusters"], callback=self.update_method,
        )

        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2840, "Fixed:"], addToLayout=False), 1, 1)
        ...
        layout.addWidget(
            gui.appendRadioButton(bg, _tr.m[2841, "From"], addToLayout=False), 2, 1)
        ...
        gui.widgetLabel(ftobox, _tr.m[2842, "to"])
        gui.spin(
        ...
        box = gui.vBox(self.controlArea, _tr.m[2843, "Preprocessing"])
        gui.checkBox(box, self, "normalize", _tr.m[2844, "Normalize columns"],
                     callback=self.invalidate)

        box = gui.vBox(self.controlArea, _tr.m[2845, "Initialization"])
        gui.comboBox(
        ...
        layout.addWidget(gui.widgetLabel(None, _tr.m[2846, "Re-runs: "]), 0, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        layout.addWidget(
            gui.widgetLabel(None, _tr.m[2847, "Maximum iterations: "]), 1, 0, Qt.AlignLeft)
        sb = gui.hBox(None, margin=0)
        ...
        box = gui.vBox(self.mainArea, box=_tr.m[2848, "Silhouette Scores"])
        if self.optimize_k:
    ...
    def send_data(self):
        ...
        cluster_var = DiscreteVariable(
            get_unique_names(domain, _tr.m[2849, "Cluster"]),
            values=[_tr.m[2850, "C%d"] % (x + 1) for x in range(km.k)],
            compute_value=km
        )
        silhouette_var = ContinuousVariable(
            get_unique_names(domain, _tr.m[2851, "Silhouette"]))
        ...
        if self.data.name == Table.name:
            centroids.name = _tr.m[2852, "centroids"]
        else:
            centroids.name = _tr.e(_tr.c(2853, f"{self.data.name} centroids"))
    ...
    def send_report(self):
        ...
        self.report_items((
            (_tr.m[2854, "Number of clusters"], k_clusters),
            (_tr.m[2855, "Optimization"], _tr.m[2856, "{}, {} re-runs limited to {} steps"].format(
                init_method, self.n_init, self.max_iterations))))
        if self.data is not None:
            self.report_data(_tr.m[2857, "Data"], self.data)
            if self.optimize_k:
                self.report_table(
                    _tr.m[2858, "Silhouette scores for different numbers of clusters"],
                    self.table_view)
