...
bool]:
    ...
    """
    state.set_status(_t("Calculating..."))
...
class OWWordCloud(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        ...
        selected_words = Output("Selected Words", Table, dynamic=False)
    ...
    class Warning(widget.OWWidget.Warning):
        topic_precedence = widget.Msg(
            _t("Input signal Topic takes priority over Corpus")
        )

    class Info(widget.OWWidget.Information):
        bow_weights = widget.Msg(_t("Showing bag of words weights."))
...
<script src="resources/wordcloud-script.js"></script>
    ...
    def _create_layout(self):
        box = gui.widgetBox(self.controlArea, _t("Cloud preferences"))
        gui.checkBox(
            box,
            self,
            "words_color",
        ...
        box = gui.widgetBox(self.controlArea, _t("Words && weights"))
        ...
        model.setHorizontalHeaderLabels([_t("Weight"), _t("Word")])
    ...
    def create_weight_list(self, document_counter):
        ...
        if self.corpus is not None:
            ...
            domain = Domain(
                [ContinuousVariable(_t("Word Count")), ContinuousVariable(
                    _t("Document Count"))],
                metas=[StringVariable(_t("Word"))],
            )
            wc_table = Table.from_numpy(domain, X=np.hstack((w_count, d_count)),
            metas=words)
            wc_table.name = _t("Word Counts")
    ...
    def commit(self):
        ...
        if words:
            ...
            words_table.name = _t("Selected Words")
