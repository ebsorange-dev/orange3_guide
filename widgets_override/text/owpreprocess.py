...
from AnyQt.QtCore import Qt, pyqtSignal, QModelIndex, QTimer
...
class FileLoader(QWidget):
    ...
    def __init__(self, kind: str = ""):
        """
        kind: PC 파일 선택기 신호 식별자 ("stopwords" / "lexicon" / "" = 사용 안 함).
              빈 값이면 기존 Qt 파일 다이얼로그 사용 (단독 사용 시 호환성 유지).
        """
        super().__init__()
        self.recent_paths = []
        self._pc_kind = (kind or "").strip()
        ...
        # PC 업로드 모드 — 신호 파일 폴링 (File 위젯과 동일 패턴).
        # /config/.upload_path_<kind> 가 생기면 add_path() + _activate() 호출.
        if self._pc_kind:
            self._upload_signal = f"/config/.upload_path_{self._pc_kind}"
            self._upload_poll_timer = QTimer(self)
            self._upload_poll_timer.setInterval(1500)
            self._upload_poll_timer.timeout.connect(self._poll_upload_path)
            self._upload_poll_timer.start()

    def browse(self):
        # PC 모드: 신호 파일 작성 → 세션매니저 SSE → 브라우저 파일 선택기 호출.
        if self._pc_kind:
            import time as _time
            req_path = f"/config/.upload_request_{self._pc_kind}"
            try:
                ...
                pass
            return
        # 기본: Qt 파일 다이얼로그 (PC 모드 미사용 시 호환)
        start_file = self.last_path() or os.path.expanduser("~/")
        formats = [_t("Text files (*.txt)"), _t("All files (*)")]
        file_name, _ = QFileDialog.getOpenFileName(
            None, _t("Open..."), start_file, ";;".join(formats), formats[0])
        if not file_name:
            return
        self.add_path(file_name)
        self._activate()

    def _poll_upload_path(self):
        """업로드 완료 신호 폴링 — /config/.upload_path_<kind> 가 생기면 소비."""
        signal = getattr(self, "_upload_signal", None)
        if not signal or not os.path.isfile(signal):
            return
        try:
    ...
    def set_current_file(self, path: str):
        if path:
            ...
            self.file_combo.setCurrentText(_t("(none)"))
...
class TokenizerModule(SingleMethodModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addWidget(QLabel(_t("Pattern:")), 3, 1)
...
class NormalizationModule(SingleMethodModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.__check_use = QCheckBox(_t("UDPipe tokenizer"),
...
class FilteringModule(MultipleMethodModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.__sw_loader = FileLoader(kind="stopwords")
        ...
        self.__lx_loader = FileLoader(kind="lexicon")
        ...
        rel_freq_rb = QRadioButton(_t("Relative:"))
        abs_freq_rb = QRadioButton(_t("Absolute:"))
...
class NgramsModule(PreprocessorModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addRow(_t("Range:"), self.__range_spins)
...
PREPROCESS_ACTIONS = [
    PreprocessAction(
        _t("Transformation"), "preprocess.transform", "",
        Description(_t("Transformation"), icon_path("Transform.svg")),
        TransformationModule
    ),
    PreprocessAction(
        _t("Tokenization"), "preprocess.tokenize", "",
        Description(_t("Tokenization"), icon_path("Tokenize.svg")),
        TokenizerModule
    ),
    PreprocessAction(
        _t("Normalization"), "preprocess.normalize", "",
        Description(_t("Normalization"), icon_path("Normalize.svg")),
        NormalizationModule
    ),
    PreprocessAction(
        _t("Filtering"), "preprocess.filter", "",
        Description(_t("Filtering"), icon_path("Filter.svg")),
        FilteringModule
    ),
    PreprocessAction(
        _t("N-grams Range"), "preprocess.ngrams", "",
        Description(_t("N-grams Range"), icon_path("NGrams.svg")),
        NgramsModule
    ),
    PreprocessAction(
        _t("POS Tagger"), "tag.pos", "",
        Description(_t("POS Tagger"), icon_path("POSTag.svg")),
        POSTaggingModule
    )
]
...
class OWPreprocess(Orange.widgets.data.owpreprocess.OWPreprocess,
    ...
    class Error(Orange.widgets.data.owpreprocess.OWPreprocess.Error):
        unknown_error = Msg("{}")
        udpipe_offline_no_models = Msg(_t("No UDPipe model is selected."))
        file_not_found = Msg(_t("File not found."))
    ...
    class Warning(Orange.widgets.data.owpreprocess.OWPreprocess.Warning):
        no_token_left = Msg(_t("No tokens on the output."))
