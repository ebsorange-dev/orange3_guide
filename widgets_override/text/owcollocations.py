...
from AnyQt.QtCore import Qt, pyqtSignal as Signal, QSize
...
SCORING_METHODS = [
    ScoreMeta(_t("Pointwise Mutual Information"), [bi_measures.pmi,
                                               tri_measures.pmi]),
    ScoreMeta(_t("Chi Square"), [bi_measures.chi_sq, tri_measures.chi_sq]),
...
class OWCollocations(OWWidget):
    ...
    def __init__(self) -> None:
        ...
        setting_box = gui.vBox(self.controlArea, box=_t("Settings"))
        gui.radioButtons(setting_box, self, "type_index",
                         btnLabels=[_t("Bigrams"), _t("Trigrams")],
                         orientation=Qt.Horizontal,
                         callback=self._change_type)

        gui.spin(setting_box, self, "freq_filter", minv=1, maxv=1000, step=1,
                 label=_t("Frequency"), callback=self.commit)

        method_box = gui.vBox(self.controlArea, box=_t("Scoring Method"))
        ...
        gui.button(self.buttonsArea, self, _t("Restore Original Order"),
                   callback=self.restore_order,
                   tooltip=_t("Show rows in the original order"),
                   autoDefault=False)

        # GUI
        self.collModel = model = BarRatioTableModel(parent=self)  # type:
        # TableModel
        model.setHorizontalHeaderLabels([_t("Collocation"), _t("Score")])
    ...
    def create_scores_table(self):
        domain = Domain([ContinuousVariable(_t("Scores"))],
        ...
        new_table.name = _t("Collocation Scores")
        return new_table
    ...
    def send_report(self):
        ...
        if self.results:
            self.report_items("Collocations", (
                (_t("N-grams"), [_t("Bigrams"), _t("Trigrams")][self.type_index]),
                (_t("Method"), self.method.name),
                (_t("Frequency"), self.freq_filter)
            ))
