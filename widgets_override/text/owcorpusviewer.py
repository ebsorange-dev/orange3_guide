...
class OWCorpusViewer(OWWidget, ConcurrentWidgetMixin):
    ...
    class Warning(OWWidget.Warning):
        no_feats_search = Msg(_t("No features included in search."))
        no_feats_display = Msg(_t("No features selected for display."))

    class Error(OWWidget.Error):
        invalid_regex = Msg(_t("Invalid regular expression."))
    ...
    def __init__(self):
        ...
        # Info attributes
        self.update_info()
        info_box = gui.widgetBox(self.controlArea, _t("Info"))
        gui.label(info_box, self, _t("Tokens: %(n_tokens)s"))
        gui.label(info_box, self, _t("Types: %(n_types)s"))
        gui.label(info_box, self, _t("Matching documents: %(n_matching)s"))
        gui.label(info_box, self, _t("Matches: %(n_matches)s"))

        # Search features
        ex_sel = QListView.ExtendedSelection
        search_box = gui.widgetBox(self.controlArea, _t("Search features"))
        ...
        # Display features
        display_box = gui.widgetBox(self.controlArea, _t("Display features"))
        ...
        # Auto-commit box
        gui.auto_commit(
            self.controlArea, self, "autocommit", _t("Send data"), _t("Auto send is on")
        )
    ...
    def send_report(self):
        self.report_items(
            (
                (_t("Query"), self.regexp_filter),
                (_t("Matching documents"), self.n_matching),
                (_t("Matches"), self.n_matches),
            )
        )
    ...
    def migrate_context(cls, context, version):
        if version < 2:
            ...
            if f_order is not None:
                f_order = f_order[0]
                # 일부 저장된 컨텍스트에서 f_order 가 list 가 아닌
                # 단일 Variable 객체로 저장된 경우 → list 로 정규화
                if not isinstance(f_order, (list, tuple)):
                    f_order = [f_order] if f_order is not None else []
