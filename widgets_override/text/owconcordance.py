...
from AnyQt.QtCore import Qt, QAbstractTableModel, QItemSelectionModel, \
    QItemSelection, QModelIndex, QSize
...
class ConcordanceModel(QAbstractTableModel):
    ...
    def get_data(self):
        domain = Domain([], metas=[StringVariable(_t("Conc. {}").format(
            self.word)), StringVariable(_t("Document"))])
        data = []
        docs = []
...
class OWConcordance(OWWidget, ConcurrentWidgetMixin):
    ...
    def __init__(self):
        ...
        gui.label(info_box, self, _t('Matching: %(n_matching)s'))

        # Width parameter
        gui.spin(self.controlArea, self, 'context_width', 3, 10, box=True,
                 label=_t("Number of words:"), callback=self.set_width)
        ...
        self.input = gui.lineEdit(
            ...
            sizePolicy=QSizePolicy(QSizePolicy.MinimumExpanding,
                                   QSizePolicy.Fixed),
            label=_t('Query:'), callback=self.set_word, callbackOnType=True)
        self.input.setFocus()
        ...
        # Auto-commit box
        gui.auto_commit(self.controlArea, self, 'autocommit', _t('Commit'),
                        _t('Auto commit is on'))
    ...
    def send_report(self):
        ...
        self.report_items(_t("Concordances"), (
            (_t("Query"), model.word),
            (_t("Tokens"), model.n_tokens),
            (_t("Types"), model.n_types),
            (_t("Matching"), self.n_matching),
        ))
