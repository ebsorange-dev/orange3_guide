...
class OWImportDocuments(widget.OWWidget):
    ...
    def __init__(self):
        ...
        #     /config/.upload_path_documents 에 디렉터리 경로 기록 → 자동 로드) ──
        from AnyQt.QtCore import QTimer as _QTimer
        self.__pc_doc_timer = _QTimer(self)
        self.__pc_doc_timer.timeout.connect(self.__pc_doc_check)
        self.__pc_doc_timer.start(1000)
    ...
    def __runOpenDialog(self):
        ...
        브라우저가 /config/.upload_request_documents 신호를 감지하여 webkitdirectory
        파일 입력을 트리거 → 사용자 PC 의 폴더 선택 → 모든 문서 업로드 →
        /config/.upload_path_documents 에 경로 기록 → __pc_doc_check 가 setCurrentPath 호출."""
        import time as _time
        try:
            with open("/config/.upload_request_documents", "w") as _f:
            ...
            dirpath = QFileDialog.getExistingDirectory(
                self, "Select Top Level Folder", startdir
            )
            if dirpath:
                self.setCurrentPath(dirpath)
                self.start()

    def __pc_doc_check(self):
        """1초 폴링 — 브라우저가 PC 폴더 업로드를 완료하면 /config/.upload_path_documents 에
        업로드 디렉터리 경로를 기록한다. 감지하면 setCurrentPath + start 호출."""
        signal = "/config/.upload_path_documents"
