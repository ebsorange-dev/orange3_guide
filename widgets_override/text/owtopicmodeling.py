...
from AnyQt.QtCore import pyqtSignal, QSize, Qt
...
class LdaWidget(TopicWidget):
    ...
    parameters = (
        ('num_topics', _t('Number of topics'), 1, 500, 1, int),
    )
...
class HdpWidget(TopicWidget):
    ...
    spin_format = '{description}:'
    parameters = (
        ('gamma', _t('First level concentration (γ)'), .1, 10, .5, float),
        ('alpha', _t('Second level concentration (α)'), 1, 10, 1, int),
        ('eta', _t('The topic Dirichlet (α)'), 0.001, .5, .01, float),
        ('T', _t('Top level truncation level (Τ)'), 10, 150, 1, int),
        ('K', _t('Second level truncation level (Κ)'), 1, 50, 1, int),
        ('kappa', _t('Learning rate (κ)'), .1, 10., .1, float),
        ('tau', _t('Slow down parameter (τ)'), 16., 256., 1., float),
    )
...
class OWTopicModeling(OWWidget, ConcurrentWidgetMixin):
    ...
    class Warning(OWWidget.Warning):
        less_topics_found = Msg(_t('Less topics found than requested.'))
    ...
    def __init__(self):
        ...
        # Commit button
        gui.auto_commit(self.buttonsArea, self, 'autocommit', _t('Commit'), box=False)
        ...
        for i, (method, attr_name) in enumerate(self.methods):
            widget = method(self, title=_t('Options'))
        ...
        box = gui.vBox(self.controlArea, _t("Topic evaluation"))
        gui.label(box, self, _t("Log perplexity: %(perplexity)s"))
        gui.label(box, self, _t("Topic coherence: %(coherence)s"))
    ...
    def on_done(self, corpus):
        ...
        if self.model.name == _t("Latent Dirichlet Allocation"):
    ...
    def send_report(self):
        ...
        if self.corpus is not None:
            self.report_items(_t('Topics'), self.topic_desc.report())
...
class TopicViewer(QTreeWidget):
    ...
    columns = [_t('Topic'), _t('Topic keywords')]
