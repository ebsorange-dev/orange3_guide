...
class Sources:
    ...
    TOKENS = _t("Tokens")  # tokens or ngrams - depending on statistic
...
def words_count(
    ...
    # [1, 2, 3] -> [[1], [2], [3]]
    return np.c_[[num_words(d, callback) for d in corpus.documents]], [_t("Word count")]
...
def characters_count(
    ...
    return np.c_[[char_count(d, callback) for d in source]], [_t("Character count")]
...
def n_gram_count(
    ...
    return np.c_[list(map(ng_count, corpus.ngrams))], [_t("N-gram count")]
...
def per_cent_unique_words(
    ...
    return np.c_[list(map(perc_unique, corpus.ngrams))], [_t("% unique words")]
...
STATISTICS = [
    ...
    (_t("Word count"), words_count, None, (Sources.DOCUMENTS,)),
    (_t("Character count"), characters_count, None, (Sources.DOCUMENTS, Sources.TOKENS)),
    (_t("N-gram count"), n_gram_count, None, (Sources.TOKENS,)),
    (_t("Average term length"), average_word_len, None, (Sources.DOCUMENTS, Sources.TOKENS)),
    (_t("Punctuation count"), punctuation_count, None, (Sources.DOCUMENTS,)),
    (_t("Capital letter count"), capital_count, None, (Sources.DOCUMENTS,)),
    (_t("Vowel count"), vowel_count, "a,e,i,o,u", (Sources.DOCUMENTS,)),
    (
        _t("Consonant count"),
        consonant_count,
    ...
    (_t("Per cent unique terms"), per_cent_unique_words, None, (Sources.TOKENS,)),
    (_t("Starts with"), starts_with, "", (Sources.TOKENS,)),
    (_t("Ends with"), ends_with, "", (Sources.TOKENS,)),
    (_t("Contains"), contains, "", (Sources.DOCUMENTS, Sources.TOKENS)),
    (_t("Regex"), regex, "", (Sources.DOCUMENTS, Sources.TOKENS)),
    (_t("POS tag"), pos_tags, "NN,VV,JJ", (Sources.TOKENS,)),
    ("Yule's I", yule, None, (Sources.TOKENS,)),
    (_t("LIX index"), lix, None, (Sources.TOKENS,)),
]
...
class OWStatistics(OWWidget, ConcurrentWidgetMixin):
    ...
    def _init_controls(self) -> None:
        ...
        gui.button(self.buttonsArea, self, _t("Apply"), callback=self.apply)
    ...
    def _init_statistics_box(self) -> None:
        ...
        grid.addWidget(QLabel(_t("Feature")), 0, 1)
        grid.addWidget(QLabel(_t("Pattern")), 0, 2)
        grid.addWidget(QLabel(_t("Compute for")), 0, 3)
    ...
    def migrate_settings(cls, settings: Dict, version: int):
        def def_source(idx):
            ...
            if STATISTICS_NAMES[idx] == _t("Regex"):
