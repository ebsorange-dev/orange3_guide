...
class OWMap(OWDataProjectionWidget):
    ...
    name = _tr.m[107, "Geo Map"]
    description = _tr.m[108, 'Show data points on a world map.']
    icon = "icons/GeoMap.svg"
    priority = 100
    keywords = _tr.m[109, "geo map, map, geo"]
    ...
    class Error(OWDataProjectionWidget.Error):
        no_lat_lon_vars = Msg(_tr.m[110, "Data has no latitude and longitude variables."])

    class Warning(OWDataProjectionWidget.Warning):
        missing_coords = Msg(
            (_tr.m[111, "Plot cannot be displayed because '{}' or '{}' "] + _tr.m[112, "is missing for all data points"]))
        out_of_range = Msg(_tr.m[113, "Points with out of range latitude or longitude are not displayed."])
        no_internet = Msg((_tr.m[114, "Cannot fetch map from the internet. "] + _tr.m[115, "Displaying only cached parts."]))

    class Information(OWDataProjectionWidget.Information):
        missing_coords = Msg(
            _tr.m[116, "Points with missing '{}' or '{}' are not displayed"])
    ...
    def _add_controls(self):
        ...
        lat_lon_box = gui.vBox(self.controlArea, box=_tr.m[117, "Layout"], spacing=0)
        options = dict(
            labelWidth=75, orientation=Qt.Horizontal, sendSelectedValue=True,
            contentsLength=14
        )

        gui.comboBox(lat_lon_box, self, 'graph.tile_provider_key', label=_tr.m[118, 'Map:'],
                     items=list(TILE_PROVIDERS),
                     callback=self.update_tile_provider, **options)

        gui.comboBox(lat_lon_box, self, 'attr_lat', label=_tr.m[119, 'Latitude:'],
                     callback=self.setup_plot,
                     model=self.lat_lon_model, **options,
                     searchable=True)

        gui.comboBox(lat_lon_box, self, 'attr_lon', label=_tr.m[120, 'Longitude:'],
                     callback=self.setup_plot,
                     model=self.lat_lon_model, **options,
                     searchable=True)
        ...
        gui.checkBox(
            self._plot_box, self,
            value="graph.freeze",
            label=_tr.m[121, "Freeze map"],
            tooltip=_tr.m[122, "If checked, the map won't change position to fit new data."])
