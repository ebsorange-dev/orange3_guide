...
class ChoroplethItem(pg.GraphicsObject):
    ...
    def _get_region_info(region: _ChoroplethRegion):
        return _tr.m[11, "<b>Region = </b>"] + region.info['name']

    def tooltip(self):
        return (f"<b>{self.agg_func} = {self.agg_value}</b><hr/>" + f"{self._region_info}")
...
class OWChoroplethPlotGraph(gui.OWComponent, QObject):
    ...
    def _create_drag_tooltip(self, scene):
        tip_parts = [
            (Qt.ControlModifier,
             _tr.m[12, "{}: Append to group"].
             format("Cmd" if sys.platform == "darwin" else "Ctrl")),
            (Qt.ShiftModifier, _tr.m[13, "Shift: Add group"]),
...
AGG_FUNCS = {
    _tr.m[14, 'Instance Count']: AggDesc("size", True, True),
    _tr.m[15, 'Defined Values Count']: AggDesc("count", True, True),
    _tr.m[16, 'Sum']: AggDesc("sum", False, False),
    _tr.m[17, 'Mean']: AggDesc("mean", False, True),
    _tr.m[18, 'Median']: AggDesc("median", False, True),
    _tr.m[19, 'Mode']: AggDesc(lambda x: stats.mode(x, nan_policy='omit', keepdims=True).mode[0],
                    True, True),
    _tr.m[20, 'Maximal']: AggDesc("max", False, True),
    _tr.m[21, 'Minimal']: AggDesc("min", False, True),
    _tr.m[22, 'Std.']: AggDesc("std", False, False)
}

DEFAULT_AGG_FUNCS = {
    DiscreteVariable: _tr.m[23, "Mode"],
    ContinuousVariable: _tr.m[24, "Mean"],
    TimeVariable: _tr.m[25, "Median"]
}
...
class OWChoropleth(OWWidget):
    ...
    name = _tr.m[26, "Choropleth Map"]
    description = (_tr.m[27, 'A thematic map in which areas are shaded in proportion '] + _tr.m[28, 'to the measurement of the statistical variable being displayed.'])
    icon = "icons/Choropleth.svg"
    priority = 120
    keywords = _tr.m[29, "choropleth map, geo"]

    class Inputs:
        data = Input(_tr.m[30, "Data"], Table, id="Data", default=True)

    class Outputs:
        selected_data = Output(_tr.m[31, "Selected Data"], Table, id="Selected Data", default=True)
    ...
    class Error(OWWidget.Error):
        no_lat_lon_vars = Msg(_tr.m[32, "Data has no latitude and longitude variables."])

    class Warning(OWWidget.Warning):
        no_region = Msg(_tr.m[33, "{} points are not in any region."])
    ...
    def _add_controls(self):
        ...
        lat_lon_box = gui.vBox(self.controlArea, box=_tr.m[34, "Map settings"], spacing=0)
        self.lat_lon_model = DomainModel(DomainModel.MIXED,
                                         valid_types=(ContinuousVariable,))
        gui.comboBox(lat_lon_box, self, 'attr_lat', label=_tr.m[35, 'Latitude:'],
                     callback=self.setup_plot, model=self.lat_lon_model,
                     **options, searchable=True)

        gui.comboBox(lat_lon_box, self, 'attr_lon', label=_tr.m[36, 'Longitude:'],
                     callback=self.setup_plot, model=self.lat_lon_model,
                     **options, searchable=True)

        a_slider = gui.hSlider(lat_lon_box, self, 'admin_level', minValue=0,
                               maxValue=2, step=1, label=_tr.m[37, 'Detail:'],
                               createLabel=False, callback=self.setup_plot)
        a_slider.setFixedWidth(176)

        agg_box = gui.vBox(self.controlArea, box=_tr.m[38, "Controls"], spacing=0)
        self.agg_attr_model = DomainModel(valid_types=(ContinuousVariable,
                                                       DiscreteVariable))
        gui.comboBox(agg_box, self, 'agg_attr', label=_tr.m[39, 'Values:'],
        ...
        b_slider = gui.hSlider(visualization_box, self, "binning_index",
                               label=_tr.m[41, "Bin width:"], minValue=0,
    ...
    def send_data(self):
        ...
        if data:
            ...
            if np.sum(graph_sel) > 0:
                selected_data = create_groups_table(data, group_sel, False, _tr.m[44, "Group"])
