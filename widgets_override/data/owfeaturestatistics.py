...
def format_time_diff(start, end, round_up_after=2):
    ...
    if years >= round_up_after:
        return _tr.e(_tr.c(839, f'~{years} years'))
    elif months >= round_up_after:
        return _tr.e(_tr.c(840, f'~{months} months'))
    elif weeks >= round_up_after:
        return _tr.e(_tr.c(841, f'~{weeks} weeks'))
    elif days >= round_up_after:
        return _tr.e(_tr.c(842, f'~{days} days'))
    elif hours >= round_up_after:
        return _tr.e(_tr.c(843, f'~{hours} hours'))
    elif minutes >= round_up_after:
        return _tr.e(_tr.c(844, f'~{minutes} minutes'))
    else:
        return _tr.e(_tr.c(845, f'{seconds} seconds'))
...
class FeatureStatisticsTableModel(AbstractSortTableModel):
    ...
    class Columns(IntEnum):
        ...
        def name(self):
            return {self.ICON: '',
                    self.NAME: _tr.m[846, 'Name'],
                    self.DISTRIBUTION: _tr.m[847, 'Distribution'],
                    self.CENTER: _tr.m[848, 'Mean'],
                    self.MODE: _tr.m[849, 'Mode'],
                    self.MEDIAN: _tr.m[850, 'Median'],
                    self.DISPERSION: _tr.m[851, 'Dispersion'],
                    self.MIN: _tr.m[852, 'Min.'],
                    self.MAX: _tr.m[853, 'Max.'],
                    self.MISSING: _tr.m[854, 'Missing'],
                    }[self.value]
    ...
    def get_statistics_table(self):
        ...
        if any(contivars):
            ...
            attrs = [ContinuousVariable(name)
                     for name in (_tr.m[855, "Entropy"], self.Columns.MISSING.name)]
        ...
        meta_attrs = [StringVariable(_tr.m[856, 'Feature']), StringVariable(_tr.m[857, 'Mode'])]

        domain = Domain(attributes=attrs, metas=meta_attrs)
        statistics = Table.from_numpy(domain, x, metas=metas)
        statistics.name = _tr.e(_tr.c(858, f'{self.table.name} (Feature Statistics)'))
        return statistics
...
class OWFeatureStatistics(widget.OWWidget):
    name = _tr.m[859, 'Feature Statistics']
    description = _tr.m[860, 'Show basic statistics for data features.']
    icon = 'icons/FeatureStatistics.svg'

    class Inputs:
        data = Input(_tr.m[861, 'Data'], Table, id="Data", default=True)

    class Outputs:
        reduced_data = Output(_tr.m[862, 'Reduced Data'], Table, id="Reduced Data", default=True)
        statistics = Output(_tr.m[863, 'Statistics'], Table, id="Statistics")
