...
from AnyQt.QtWidgets import \
    ...
    QLineEdit, QSizePolicy as Policy, QCompleter, QWidget
from AnyQt.QtCore import Qt, QTimer, QSize, QUrl, QEvent, QByteArray
from AnyQt.QtGui import QBrush, QIcon, QPixmap
...
DEFAULT_READER_TEXT = _tr.m[866, "Automatically detect type"]
...
class OWFile(OWUrlDropBase, RecentPathsWComboMixin):
    ...
    description = (_tr.m[868, "Read data from an input file or network "] + _tr.m[869, "and send a data table to the output."])
    icon = "icons/File.svg"
    priority = 10
    category = _tr.m[870, "Data"]
    keywords = _tr.m[871, "file, load, read, open"]

    class Outputs:
        data = Output(_tr.m[872, "Data"], Table, id="Data",
                      doc=_tr.m[873, "Attribute-valued dataset read from the input file."])
    ...
    class Information(widget.OWWidget.Information):
        no_file_selected = Msg(_tr.m[874, "No file selected."])

    class Warning(widget.OWWidget.Warning):
        file_too_big = Msg((_tr.m[875, "The file is too large to load automatically."] + _tr.m[876, " Press Reload to load."]))
        load_warning = Msg(_tr.m[877, "Read warning:\n{}"])
        performance_warning = Msg(
            _tr.m[878, "Categorical variables with >100 values may decrease performance."])
        renamed_vars = Msg((_tr.m[879, "Some variables have been renamed "] + _tr.m[880, "to avoid duplicates.\n{}"]))
        multiple_targets = Msg(_tr.m[881, "Most widgets do not support multiple targets"])

    class Error(widget.OWWidget.Error):
        file_not_found = Msg(_tr.m[882, "File not found."])
        missing_reader = Msg(_tr.m[883, "Missing reader."])
        sheet_error = Msg(_tr.m[884, "Error listing available sheets."])
        unknown = Msg(_tr.m[885, "Read error:\n{}"])

    # CSV File Import 추천 알림 + tabular data 안내 알림 비노출
    UserAdviceMessages = []

    def sizeHint(self):
        return QSize(500, 620)

    def __init__(self):
        super().__init__()
        self.setWindowFlags(
            (self.windowFlags() & ~Qt.Window & ~Qt.WindowCloseButtonHint)
            | Qt.Dialog | Qt.WindowMaximizeButtonHint
        )
        # 상태바 버튼(≡, ?, □) 숨기기
        sb = self.statusBar()
        buttons_widget = sb.findChild(QWidget, "buttons")
        if buttons_widget:
            buttons_widget.hide()
        ...
        layout = QGridLayout()
        layout.setSpacing(4)
        gui.widgetBox(self.controlArea, orientation=layout, box=_tr.m[891, 'Source'])
        vbox = gui.radioButtons(None, self, "source", box=True,
                                callback=self.load_data, addToLayout=False)

        rb_button = gui.appendRadioButton(vbox, _tr.m[892, "File:"], addToLayout=False)
        ...
        file_button = gui.button(
            None, self, '...', callback=self._request_pc_upload, autoDefault=False)
        file_button.setIcon(self.style().standardIcon(QStyle.SP_DirOpenIcon))
        file_button.setSizePolicy(Policy.Maximum, Policy.Fixed)
        layout.addWidget(file_button, 0, 2)

        # ── Dataset 버튼 ── 공용 데이터 카탈로그(orange3_datasets_compact.html) 호출
        dataset_button = gui.button(
            None, self, "Dataset",
            callback=self._request_dataset_catalog, autoDefault=False)
        dataset_button.setIcon(self._make_dataset_icon())
        dataset_button.setSizePolicy(Policy.Fixed, Policy.Fixed)
        dataset_button.setToolTip("공용 데이터셋 카탈로그 열기")
        layout.addWidget(dataset_button, 0, 3)

        reload_button = gui.button(
            None, self, _tr.m[893, "Reload"], callback=self.load_data, autoDefault=False)
        reload_button.setIcon(self.style().standardIcon(
            QStyle.SP_BrowserReload))
        reload_button.setSizePolicy(Policy.Fixed, Policy.Fixed)
        layout.addWidget(reload_button, 0, 4)

        # 업로드 디렉터리 폴링 설정 (Docker 바인드 마운트는 inotify 미지원)
        self._upload_dir = "/config/uploads"
        os.makedirs(self._upload_dir, exist_ok=True)
        try:
            self._known_uploads = set(os.listdir(self._upload_dir))
        except OSError:
            self._known_uploads = set()
        self._upload_poll_timer = QTimer(self)
        self._upload_poll_timer.setInterval(1500)  # 1.5초 주기
        self._upload_poll_timer.timeout.connect(self._poll_uploads)
        ...
        self.sheet_label.setText(_tr.m[894, 'Sheet'])
        ...
        layout = QGridLayout()
        layout.setSpacing(4)
        gui.widgetBox(self.controlArea, orientation=layout, box=_tr.m[895, 'File Type'])
        ...
        self.infolabel = gui.widgetLabel(box, _tr.m[896, 'No data loaded.'])

        box = gui.widgetBox(self.controlArea, _tr.m[897, "Columns (Double click to edit)"])
        ...
        box = gui.hBox(box)
        gui.button(
            box, self, _tr.m[898, "Reset"], callback=self.reset_domain_edit,
            autoDefault=False
        )
        gui.rubber(box)
        self.apply_button = gui.button(
            box, self, _tr.m[899, "Apply"], callback=self.apply_domain_edit)
    ...
    def _request_pc_upload(self):
        """브라우저 파일 선택 요청 — 신호 파일을 /config/.upload_request 에 기록."""
        import time as _time
        os.makedirs(self._upload_dir, exist_ok=True)
        try:
            with open("/config/.upload_request", "w") as f:
    ...
    def _make_dataset_icon(self):
        ...
        pix = QPixmap()
        pix.loadFromData(QByteArray(svg), "SVG")
        return QIcon(pix)

    def _request_dataset_catalog(self):
        """Dataset 버튼 — 공용 데이터셋 카탈로그(HTML) 호출 신호.
        세션 매니저가 /dataset-poll 로 감지 후 인페이지 모달로 표시."""
        import time as _time
        try:
            with open("/config/.dataset_request", "w") as f:
                f.write(str(_time.time()))
        except OSError as e:
            # 신호 파일 작성 실패 시에만 경고 표시
            QMessageBox.warning(
                self, "Dataset",
                f"카탈로그 호출 신호 작성 실패:\n{e}")

    def _poll_uploads(self):
        """1.5초 주기 폴링 — .upload_path 신호 파일 감지 후 자동 로드.
        세션 매니저가 Docker API로 /tmp/ 에 복사 후 경로를 신호 파일에 기록."""
        signal = "/config/.upload_path"
        ...
        if in_demos:
            ...
            if not os.path.exists(start_file):
                QMessageBox.information(
                    None, _tr.m[901, "File"],
                    _tr.m[902, "Cannot find the directory with documentation datasets"])
                return
        else:
            start_file = self.last_path() or "/data"
    ...
    def load_data(self):
        ...
        if error:
            ...
            self.infolabel.setText(_tr.m[903, "No data."])
        ...
        if self.source == self.LOCAL_FILE:
            ...
            if self.recent_paths and self.recent_paths[0].file_format:
                ...
                try:
                    ...
                    raise MissingReaderException(_tr.e(_tr.c(904, f'Can not find reader "{qname}"'))) from ex
    ...
    def _describe(table):
        ...
        descs = [attrs[desc]
                 for desc in (_tr.m[905, "Name"], _tr.m[906, "Description"]) if desc in attrs]
        if len(descs) == 2:
            descs[0] = f"<b>{descs[0]}</b>"
        if descs:
            text += f"<p>{'<br/>'.join(descs)}</p>"

        text += _tr.e(_tr.c(907, f"<p>{len(table)} {pl(len(table), 'instance')}"))
        ...
        text += _tr.e(_tr.c(908, f"<br/>{nattrs} {pl(nattrs, 'feature')} {missing_in_attr}"))
        if domain.has_continuous_class:
            text += _tr.e(_tr.c(909, f"<br/>Regression; numerical class {missing_in_class}"))
        elif domain.has_discrete_class:
            nvals = len(domain.class_var.values)
            text += (_tr.m[910, "<br/>Classification; categorical class "] + _tr.e(_tr.c(911, f"with {nvals} {pl(nvals, 'value')} {missing_in_class}")))
        elif table.domain.class_vars:
            ntargets = len(table.domain.class_vars)
            text += (_tr.m[912, "<br/>Multi-target; "] + (_tr.e(_tr.c(913, f"{ntargets} target {pl(ntargets, 'variable')} ")) + f"{missing_in_class}"))
        else:
            text += _tr.m[914, "<br/>Data has no target variable."]
        nmetas = len(domain.metas)
        text += _tr.e(_tr.c(915, f"<br/>{nmetas} {pl(nmetas, 'meta attribute')}"))
        text += "</p>"

        if 'Timestamp' in table.domain:
            # Google Forms uses this header to timestamp responses
            text += (_tr.e(_tr.c(916, f"<p>First entry: {table[0, 'Timestamp']}<br/>")) + _tr.e(_tr.c(917, f"Last entry: {table[-1, 'Timestamp']}</p>")))
        return text
    ...
    def apply_domain_edit(self):
        ...
        if self.data is None:
            ...
            if renamed:
                self.Warning.renamed_vars(_tr.e(_tr.c(918, f"Renamed: {', '.join(renamed)}")))
    ...
    def send_report(self):
        def get_ext_name(filename):
            try:
                ...
                return _tr.m[919, "unknown"]

        if self.data is None:
            self.report_paragraph(_tr.m[920, "File"], _tr.m[921, "No file."])
            return
        ...
        if self.source == self.LOCAL_FILE:
            ...
            self.report_items(_tr.m[922, "File"], [(_tr.m[923, "File name"], name),
                                       (_tr.m[924, "Format"], get_ext_name(name))])
        else:
            self.report_items(_tr.m[925, "Data"], [(_tr.m[926, "Resource"], self.url),
                                       (_tr.m[927, "Format"], get_ext_name(self.url))])

        self.report_data(_tr.m[928, "Data"], self.data)
