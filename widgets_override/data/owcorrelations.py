...
class CorrelationType(IntEnum):
    ...
    def items():
        ...
        """
        return [_tr.m[392, "Pearson correlation"], _tr.m[393, "Spearman correlation"]]
...
class OWCorrelations(OWWidget):
    name = _tr.m[394, "Correlations"]
    description = _tr.m[395, "Compute all pairwise attribute correlations."]
    icon = "icons/Correlations.svg"
    priority = 1106
    category = _tr.m[396, "Unsupervised"]

    class Inputs:
        data = Input(_tr.m[397, "Data"], Table, id="Data")

    class Outputs:
        data = Output(_tr.m[398, "Data"], Table, id="Data")
        features = Output(_tr.m[399, "Features"], AttributeList, id="Features")
        correlations = Output(_tr.m[400, "Correlations"], Table, id="Correlations", dynamic=False)
    ...
    class Information(OWWidget.Information):
        removed_cons_feat = Msg(_tr.m[401, "Constant features have been removed."])

    class Warning(OWWidget.Warning):
        not_enough_vars = Msg(_tr.m[402, "At least two numeric features are needed."])
        not_enough_inst = Msg(_tr.m[403, "At least two instances are needed."])
    ...
    def __init__(self):
        ...
        self.feature_model = DomainModel(
            ...
            placeholder=_tr.m[404, "(All combinations)"], valid_types=ContinuousVariable)
        gui.comboBox(
    ...
    def commit(self):
        ...
        attrs = [ContinuousVariable(_tr.m[405, "Correlation"]),
                 ContinuousVariable(_tr.m[406, "uncorrected p"]),
                 ContinuousVariable(_tr.m[407, "FDR"])]
        metas = [StringVariable(_tr.m[408, "Feature 1"]), StringVariable(_tr.m[409, "Feature 2"])]
        ...
        corr_table.name = _tr.m[410, "Correlations"]
