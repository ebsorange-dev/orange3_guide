...
# Default scores.
CLS_SCORES = [
    ScoreMeta(_tr.m[1413, "Information Gain"], _tr.m[1414, "Info. gain"],
              score.InfoGain, ProblemType.CLASSIFICATION, False),
    ScoreMeta(_tr.m[1415, "Information Gain Ratio"], _tr.m[1416, "Gain ratio"],
              score.GainRatio, ProblemType.CLASSIFICATION, True),
    ScoreMeta(_tr.m[1417, "Gini Decrease"], "Gini",
...
REG_SCORES = [
    ScoreMeta(_tr.m[1418, "Univariate Regression"], _tr.m[1419, "Univar. reg."],
...
class OWRank(OWWidget, ConcurrentWidgetMixin):
    name = _tr.m[1420, "Rank"]
    description = _tr.m[1421, "Rank and filter data features by their relevance."]
    icon = "icons/Rank.svg"
    priority = 1102
    keywords = _tr.m[1422, "rank, filter"]

    buttons_area_orientation = Qt.Vertical

    class Inputs:
        data = Input(_tr.m[1423, "Data"], Table, id="Data")
        scorer = MultiInput(_tr.m[1424, "Scorer"], score.Scorer, id="Scorer", filter_none=True)

    class Outputs:
        reduced_data = Output(_tr.m[1425, "Reduced Data"], Table, id="Reduced Data", default=True)
        scores = Output(_tr.m[1426, "Scores"], Table, id="Scores", dynamic=False)
        features = Output(_tr.m[1427, "Features"], AttributeList, id="Features", dynamic=False)
    ...
    class Information(OWWidget.Information):
        no_target_var = Msg(_tr.m[1428, "Data does not have a (single) target variable."])
        missings_imputed = Msg(_tr.m[1429, 'Missing values will be imputed as needed.'])

    class Error(OWWidget.Error):
        invalid_type = Msg(_tr.m[1430, "Cannot handle target variable type {}"])
        inadequate_learner = Msg(_tr.m[1431, "Scorer {} inadequate: {}"])
        no_attributes = Msg(_tr.m[1432, "Data does not have a single attribute."])

    class Warning(OWWidget.Warning):
        renamed_variables = Msg(
            _tr.m[1433, "Variables with duplicated names have been renamed."])
    ...
    def __init__(self):
        ...
        for scoring_methods in (CLS_SCORES,
                                REG_SCORES,
                                []):
            box = gui.vBox(None, _tr.m[1434, "Scoring Methods"] if scoring_methods else None)
        ...
        selMethBox = gui.vBox(self.buttonsArea, _tr.m[1435, "Select Attributes"])
        ...
        b1 = button(self.tr(_tr.m[1436, "None"]), OWRank.SelectNone)
        b2 = button(self.tr(_tr.m[1437, "All"]), OWRank.SelectAll)
        b3 = button(self.tr(_tr.m[1438, "Manual"]), OWRank.SelectManual)
        b4 = button(self.tr(_tr.m[1439, "Best ranked:"]), OWRank.SelectNBest)
    ...
    def handleNewSignals(self):
        self.setStatusMessage(_tr.m[1440, 'Running'])
        self.update_scores()
    ...
    def send_report(self):
        if not self.data:
            return
        self.report_domain(_tr.m[1441, "Input"], self.data.domain)
        self.report_table(_tr.m[1442, "Ranks"], self.ranksView, num_format="{:.3f}")
        if self.out_domain_desc is not None:
            self.report_items(_tr.m[1443, "Output"], self.out_domain_desc)
    ...
    def create_scores_table(self, labels):
        ...
        if not model_list or len(model_list[0]) == 2:  # Empty or just first two columns
            return None
        unique, renamed = get_unique_names_duplicates(labels + (_tr.m[1444, 'Feature'],),
        ...
        new_table.name = _tr.m[1445, "Feature Scores"]
        return new_table
