    ...
    def from_dict(cls, var, data):
        ...
        if val_map is not None:
            ...
            if len(set(mapped_values)) != len(mapped_values):
                warnings.append(
                    (f"{var.name}: " + _tr.m[247, "renaming of values ignored due to duplicate names"]))
            else:
...
class OWColor(widget.OWWidget):
    name = _tr.m[249, "Color"]
    description = _tr.m[250, "Set color legend for variables."]
    icon = "icons/Colors.svg"

    class Inputs:
        data = Input(_tr.m[251, "Data"], Orange.data.Table, id="Data")

    class Outputs:
        data = Output(_tr.m[252, "Data"], Orange.data.Table, id="Data")
    ...
    def __init__(self):
        ...
        box = gui.hBox(self.controlArea, _tr.m[253, "Discrete Variables"])
        ...
        box = gui.hBox(self.controlArea, _tr.m[254, "Numeric Variables"])
        ...
        gui.button(box, self, _tr.m[255, "Save"], callback=self.save)
        gui.button(box, self, _tr.m[256, "Load"], callback=self.load)
        gui.button(box, self, _tr.m[257, "Reset"], callback=self.reset)
    ...
    def save(self):
        fname, _ = QFileDialog.getSaveFileName(
            self, _tr.m[258, "File name"], self._start_dir(),
            _tr.m[259, "Variable definitions (*.colors)"])
        if not fname:
            return
    ...
    def load(self):
        fname, _ = QFileDialog.getOpenFileName(
            self, _tr.m[260, "File name"], self._start_dir(),
            _tr.m[261, "Variable definitions (*.colors)"])
        if not fname:
            return
        ...
        try:
            ...
            QMessageBox.critical(self, _tr.m[262, "File error"], _tr.m[263, "File cannot be opened."])
            return
        except (json.JSONDecodeError, InvalidFileFormat):
            QMessageBox.critical(self, _tr.m[264, "File error"], _tr.m[265, "Invalid file format."])
            return
    ...
    def _parse_var_defs(self, js):
        ...
        if len(renamed_vars) != len(self.disc_descs) + len(self.cont_descs):
            QMessageBox.warning(
                self,
                _tr.m[266, "Duplicated variable names"],
                _tr.m[267, "Variables will not be renamed due to duplicated names."])
        ...
        if unused_vars:
            ...
            if len(unused_vars) == 1:
                warn = (_tr.e(_tr.c(268, f'Definition for variable {names[0]}, which does not ')) + _tr.e(_tr.c(269, f'appear in the data, was ignored.\n')))
            else:
                if len(unused_vars) <= 5:
                    warn = (_tr.e(_tr.c(270, 'Definitions for variables ')) + _tr.e(_tr.c(271, f'{", ".join(names[:-1])} and {names[-1]}')))
                else:
                    warn = (_tr.e(_tr.c(272, f'Definitions for {", ".join(names[:4])} ')) + _tr.e(_tr.c(273, f'and {len(names) - 4} other variables')))
                warn += _tr.m[274, ", which do not appear in the data, were ignored.\n"]
        ...
        if warnings:
            QMessageBox.warning(
                self, _tr.m[275, "Invalid definitions"], "\n".join(warnings))
    ...
    def send_report(self):
        ...
        def _report_variables(variables):
            def was(n, o):
                return n if n == o else _tr.e(_tr.c(276, f"{n} (was: {o})"))
        ...
        sections = (
            ...
            for name, variables in (
                (_tr.m[277, "Features"], dom.attributes),
                (_tr.e(_tr.c(278, f'{pl(len(dom.class_vars), "Outcome")}')), dom.class_vars),
                (_tr.m[279, "Meta attributes"], dom.metas)))
